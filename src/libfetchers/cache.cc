 ...
 */
static void cleanupStaleLockFiles(const std::filesystem::path & lockDir, std::chrono::hours maxAge = std::chrono::hours(24))
{
    try {
        ...
        Key key, Store & store, std::function<std::pair<Attrs, StorePath>()> fetcher, unsigned int lockTimeout) override
    {
        ...
        return withFetchLock(
            ...
            [&]() -> std::optional<std::optional<ResultWithStorePath>> {
                if (auto cached = lookupStorePath(key, store))
                    return cached;
                return std::nullopt;
            },
