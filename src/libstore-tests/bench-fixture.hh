...
private:
        ...
        , store(openAtRoot(root))
    {
        ...
        /* Phase 2: parallel file I/O. Baseline port uses plain
           std::thread + atomic work-stealing counter since TBB is
           not a dep of libstore-tests at this commit. Fixture build
           is NOT what we're measuring, so any reasonable shape
           works — workers claim path indices from a shared counter
           until exhausted. */
