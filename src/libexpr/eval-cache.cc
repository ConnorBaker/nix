...
struct AttrDb
{
    ...
    [[nodiscard]] std::expected<AttrId, CacheError> setString(AttrKey key, std::string_view s, const char ** context = nullptr)
    {
        return doSQLiteWrite([&](auto & state) {
            if (context) {
                std::string ctx;
    ...
    [[nodiscard]] std::expected<AttrId, CacheError> setListOfStrings(AttrKey key, const std::vector<std::string> & l)
    {
        return doSQLiteWrite([&](auto & state) {
            state->insertAttribute
                .use()(key.first)(symbols[key.second])(AttrType::ListOfStrings)(dropEmptyInitThenConcatStringsSep("\t", l))
                .exec();
            return state->db.getLastInsertedRowId();
        });
    }
...
Value & AttrCursor::forceValue()
{
    ...
    if (root->db && (!cachedValue || std::get_if<placeholder_t>(&cachedValue->second))) {
        if (v.type() == nString) {
            if (auto id = root->db->setString(getKey(), v.c_str(), v.context()))
                cachedValue = {*id, string_t{v.c_str(), {}}};
