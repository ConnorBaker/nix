...
void EvalState::finalizeBuiltins()
{
    ...
    /* Convert the transient to an immutable ImmerBindings */
    auto * bindings = mem.allocImmerBindings(builtinsTransient_->persistent(), noPos);
...
{
    auto builder = state.buildImmerBindings(pos);
    auto dynamicEnv = &env;
...
void ExprList::eval(EvalState & state, Env & env, Value & v)
{
    ...
    // For now, always use legacy list construction
...
https://nix.dev/manual/nix/stable/language/syntax.html#functions.)",
    ...
    callFunction(fun, allocValue()->mkAttrs(builder), res, pos);
}
...
void ExprOpUpdate::eval(EvalState & state, Value & v, Value & v1, Value & v2)
{
    ...
    // Use transient for efficient batch insertion, then persist
    auto merged = v1.immerAttrs().map.transient();
    for (const auto & [name, attr] : v2.immerAttrs().map) {
        merged.set(name, attr);  // In-place mutation on transient
    }
    v.mkAttrs(state.mem.allocImmerBindings(merged.persistent(), v1.immerAttrs().pos));
...
void EvalState::assertEqValues(Value & v1, Value & v2, const PosIdx pos, std::string_view errorCtx)
{
    ...
    case nAttrs: {
        ...
        for (auto & [name, attr] : v1.immerAttrs().map)
            compareAttr(name, attr.value, attr.pos);
        return;
    }
...
bool EvalState::eqValues(Value & v1, Value & v2, const PosIdx pos, std::string_view errorCtx)
{
    ...
    case nAttrs: {
        ...
        for (auto & [name, attr] : v1.immerAttrs().map) {
            ...
            if (!attr2)
                return false;
            if (!eqValues(*attr.value, *attr2.value, pos, errorCtx))
                return false;
        }
...
void EvalState::printStatistics()
{
    ...
    // Note: ImmerBindings uses a CHAMP trie with structural sharing.
    // The actual memory usage is complex to calculate, so this is an approximation.
    uint64_t bAttrsets = memstats.nrAttrsets * sizeof(ImmerBindings) + memstats.nrAttrsInAttrsets * sizeof(AttrValue);
    ...
    topObj["sizes"] = {
        {"Env", sizeof(Env)},
        {"Value", sizeof(Value)},
        {"ImmerBindings", sizeof(ImmerBindings)},
