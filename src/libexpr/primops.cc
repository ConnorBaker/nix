    ...
    EvalState & state, const PosIdx pos, const SourcePath & path, const StorePath & storePath, Value & v)
{
    ...
    auto builder = state.buildImmerBindings();
...
{
    auto attrs = state.buildImmerBindings();
...
static void prim_attrNames(EvalState & state, const PosIdx pos, Value ** args, Value & v)
{
    ...
    size_t n = 0;
    for (const auto & [name, attrValue] : args[0]->immerAttrs().map) {
...
static void prim_attrValues(EvalState & state, const PosIdx pos, Value ** args, Value & v)
{
    ...
    for (const auto & [name, attrValue] : args[0]->immerAttrs().map) {
        pairs.emplace_back(name, attrValue.value);
    }
...
void prim_getAttr(EvalState & state, const PosIdx pos, Value ** args, Value & v)
{
    ...
    // Use attrsGet for unified access to both tAttrs and tAttrsImmer
...
static void prim_functionArgs(EvalState & state, const PosIdx pos, Value ** args, Value & v)
{
    ...
    if (args[0]->isPrimOpApp() || args[0]->isPrimOp()) {
        v.mkAttrs(&ImmerBindings::emptyImmerBindings);
        return;
    }
    if (!args[0]->isLambda())
...
static void prim_map(EvalState & state, const PosIdx pos, Value ** args, Value & v)
{
    ...
    // TODO: Immer list optimization disabled for debugging - suspected GC issue with transients
    // Always use legacy list construction
    auto list = state.buildList(len);
    for (const auto & [n, v2] : enumerate(list))
        (v2 = state.allocValue())->mkApp(args[0], args[1]->listElem(n));
    v.mkList(list);
}
...
static void prim_genList(EvalState & state, const PosIdx pos, Value ** args, Value & v)
{
    ...
    for (const auto & [n, v2] : enumerate(list)) {
        auto arg = state.allocValue();
        arg->mkInt(n);
        (v2 = state.allocValue())->mkApp(args[0], arg);
    }
    v.mkList(list);
}
...
static void prim_sort(EvalState & state, const PosIdx pos, Value ** args, Value & v)
{
    ...
    // Small list optimization: use inline storage for 1-2 elements
    // TODO: Immer list optimization disabled for debugging - suspected GC issue with transients
    // Always use legacy list construction
    auto list = state.buildList(len);
    for (const auto & [n, v2] : enumerate(list))
...
// Helper to build list from ValueVector
// TODO: Immer list optimization disabled for debugging - suspected GC issue with transients
...
static void prim_concatMap(EvalState & state, const PosIdx pos, Value ** args, Value & v)
{
    ...
    // TODO: Immer list optimization disabled for debugging - suspected GC issue
    // Always use legacy O(n) path for now
    auto list = state.buildList(len);
    auto out = list.elems;
    size_t outPos = 0;
    for (size_t n = 0; n < nrLists; ++n) {
...
void EvalState::createBaseEnv(const EvalSettings & evalSettings)
{
     ...
     * then convert to ImmerBindings in finalizeBuiltins().
     * This is necessary because ImmerBindings are immutable after creation,
    ...
    for (const auto & [n, i] : enumerate(lookupPath.elements)) {
        auto builder = buildImmerBindings();
    ...
    /* Note: Immer maps use CHAMP tries with O(1) lookup, no sorting needed.
       The legacy Bindings::sort() was required for binary search on sorted arrays. */
