...
void AttrVocabStore::feedPath(CanonicalHashBuilder & builder, AttrPathId pathId) const
{
    std::vector<std::string_view> components;
    for (auto cur = pathId; cur != rootPath(); cur = parentPath(cur))
        components.push_back(resolveName(childName(cur)));
    std::reverse(components.begin(), components.end());

    builder.field("dep.key.trace-context.attr-path.count", static_cast<uint64_t>(components.size()));
    for (auto component : components)
        builder.field("dep.key.trace-context.attr-path.component", component);
}
...
void AttrVocabStore::checkpoint()
{
    // Nothing to flush?
    bool hasNewNames = nameTable.nextId() > 0
        && (nameTable.nextId() - 1) > maxLoadedNameId;
    bool hasNewPaths = !paths.empty()
        && (static_cast<uint32_t>(paths.size()) - 1) > maxLoadedPathId;
    if (!hasNewNames && !hasNewPaths)
        return;

    // Brief independent connection — not ATTACH'd to any other DB.
    // Its own transaction ensures vocab writes are durable regardless of
    // the main DB's transaction state.
    SQLite db(dbPath, {.useWAL = settings.useSQLiteWAL});
    db.isCache();

    SQLiteStmt insertName, insertPath;
    insertName.create(db, "INSERT OR IGNORE INTO AttrNames(id, name) VALUES (?, ?)");
    insertPath.create(db, "INSERT OR IGNORE INTO AttrPaths(id, parent, child) VALUES (?, ?, ?)");

    SQLiteTxn txn(db);
    flushTo(insertName, insertPath);
    txn.commit();
}
...
void AttrVocabStore::loadFrom(SQLite & db)
{
    ...
    getAllNames.create(db, "SELECT id, name FROM AttrNames ORDER BY id");
