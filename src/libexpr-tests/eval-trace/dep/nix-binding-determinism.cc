...
TEST_F(NixBindingDeterminismTest, NixBinding_SameStringTwice_SameScopeHash)
{
...
TEST_F(NixBindingDeterminismTest, NixBinding_SameStringTwice_SameBindingHash)
{
    std::string nixCode = R"(
        let
            helper = x: x + 1;
    ...
    std::map<std::string, NixBindingHash> hashes1;
    ...
    std::map<std::string, NixBindingHash> hashes2;
...
TEST_F(NixBindingDeterminismTest, NixBinding_WithImportPath_SameScopeHash)
{
    std::string nixCode = R"(
        let
            lib = import ./lib;
...
TEST_F(NixBindingDeterminismTest, ParseFile_vs_ParseString_SameBindingHash)
{
    ...
    EXPECT_EQ(scopeHash1, scopeHash2)
        ...
        << "\n  fromFile=" << scopeHash1.value.toHex()
        << "\n  fromString=" << scopeHash2.value.toHex();
    ...
    for (auto & [name, h1] : hashes1) {
        ...
        EXPECT_EQ(h1, it->second)
            ...
            << "\n  fromFile=" << h1.value.toHex()
            << "\n  fromString=" << it->second.value.toHex();
    }
}
...
TEST_F(NixBindingDeterminismTest, NixBinding_FormalsOrder_DoesNotAffectScopeHash)
{
...
TEST_F(NixBindingDeterminismTest, NixBinding_DifferentFormals_DifferentScopeHash)
{
    auto basePath = state.rootPath(CanonPath("/test.nix"));
...
TEST_F(NixBindingDeterminismTest, NixBinding_PreInternedSymbols_StableScopeHash)
{
...
struct RelativeImportDir {
        ...
        const std::string & targetContent)
    {
        ...
        createDirs(targetPath.parent_path());
        std::ofstream(targetPath) << targetContent;
    }

    ~RelativeImportDir()
    {
        // Swallow deletePath exceptions in dtor to avoid terminate() during unwinding.
...
TEST_F(NixBindingDeterminismTest, NixBinding_WrongBasePath_ProducesDifferentHash)
{
    RelativeImportDir setup(
        "{ x = import ./sub.nix; y = 42; }",
        "sub.nix",
    ...
    EXPECT_NE(hashes1.at("x"), hashes2.at("x"))
        ...
        << "\n  fromFile=" << hashes1.at("x").value.toHex()
        << "\n  wrongBase=" << hashes2.at("x").value.toHex();
...
TEST_F(NixBindingDeterminismTest, NixBinding_DifferentRelativeImports_DifferentHash)
{
    ...
    EXPECT_NE(hash1, hash2)
        ...
        << "\n  alpha=" << hash1.value.toHex()
        << "\n  beta=" << hash2.value.toHex();
}
