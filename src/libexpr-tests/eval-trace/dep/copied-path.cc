 ...
 *
 * PATH ORIGIN MODEL: ExprConcatStrings::eval produces path values (e.g.,
 * `./dir + "/file.nix"`) without eagerly recording filesystem deps.
 * Instead, the resulting path carries PathObject provenance and the actual
 * path-using primop records the concrete dep at consumption time.
 */
...
TEST_F(DepCopiedPathTest, PathConcat_FilePath_WarmHit)
{
    // Create a directory with a file
    TempDir dir;
    dir.addFile("file.txt", "original content here");

    // Expression: ./dir + "/file.txt" — produces a path value only.
    auto expr = dir.path().string() + " + \"/file.txt\"";

    // Fresh evaluation — no filesystem dep is recorded yet.
    {
    ...
    // Verification — still serves from trace.
    {
        int loaderCalls = 0;
...
TEST_F(DepCopiedPathTest, PathConcat_FilePath_NoInvalidation)
{
    TempDir dir;
    dir.addFile("file.txt", "original content here");
    ...
    // Modify file content — the path value itself should remain valid.
    dir.addFile("file.txt", "MODIFIED content with different size!!!");
    invalidateFileCache(dir.path() / "file.txt");

    // Verification should still warm-hit because only the consumer records
    // the filesystem dep now.
    {
        int loaderCalls = 0;
        auto cache = makeCache(expr, &loaderCalls);
        auto v = forceRoot(*cache);
...
{
    // Directory path concatenation also records no eager dep.
    TempDir dir;
    dir.addSubdir("subdir");

    auto expr = dir.path().string() + " + \"/subdir\"";

    // Fresh evaluation — produces a path value; no eager dep.
    {
        auto cache = makeCache(expr);
        auto v = forceRoot(*cache);
        EXPECT_EQ(v.type(), nPath);
...
TEST_F(DepCopiedPathTest, PathConcat_ReadFile_ConsumerDep)
{
    // The actual readFile consumer records the file dep.
    TempDir dir;
    dir.addFile("data.txt", "hello from data");

    // Path concat produces a path, then readFile consumes it and records the dep.
...
TEST_F(DepCopiedPathTest, CopiedPath_MultipleTraces_AllRecordDep)
{
    ...
    // DepRecordingContext regardless of srcToStore caching.
...
// KNOWN ISSUE 2: trace-context non-transitive verification — FIXED (O1)
// Fixed: verifyTrace() now recursively verifies parent traces when
// encountering trace-context deps. verifiedTraceIds cache prevents loops.
