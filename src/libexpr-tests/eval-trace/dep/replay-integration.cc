...
TEST_F(ReplayIntegrationTest, Replay_WarmReadFile_PreservesPublication)
{
    TempTextFile file(R"({"name":"demo"})");
    auto expr = std::format(
        "{{ x = builtins.readFile {}; }}",
        file.path.string());

    SemanticHandle coldPublication;

    {
        ...
        auto publication = state.lookupSemanticHandle(*x->value);
        ASSERT_TRUE(publication.has_value());
        ASSERT_TRUE(publication->text.has_value());
        coldPublication = *publication;
    }

    {
        ...
        EXPECT_EQ(publication->text->source, coldPublication.text->source);
        EXPECT_EQ(publication->text->key, coldPublication.text->key);
        EXPECT_EQ(publication->text->contentHash, coldPublication.text->contentHash);
    }
}

TEST_F(ReplayIntegrationTest, Replay_AttachPublicationDirect_PathCarriesPathObject)
{
    TempDir dir;
    dir.addFile("demo.txt", "hello");

    auto path = state.rootPath(CanonPath((dir.path() / "demo.txt").string()));
    Value v;
    v.mkPath(path, state.mem);

    auto publication = SemanticHandle::forPath(PathObject{
        .source = DepSource::fromRuntimeRoot(runtimeRootSourceKeyFromDebugString("path:/tmp/runtime?narHash=sha256-attach")),
        .rootPath = CanonPath(dir.path().string()),
    });

    state.setSemanticHandle(v, publication);

    auto observed = state.lookupSemanticHandle(v);
    ASSERT_TRUE(observed.has_value());
    ASSERT_TRUE(observed->path.has_value());
    EXPECT_FALSE(observed->text.has_value());
    EXPECT_EQ(observed->path->source, publication.path->source);
    EXPECT_EQ(observed->path->rootPath, publication.path->rootPath);
}
...
TEST_F(ReplayDepPrecisionTest, Replay_TwoFiles_BothContentDepsRecorded)
{
    ...
    bool hasA = hasDep(deps, CanonicalQueryKind::FileBytes, fileA.path.filename().string());
    bool hasB = hasDep(deps, CanonicalQueryKind::FileBytes, fileB.path.filename().string());
    EXPECT_TRUE(hasA) << "Missing Content dep for fileA\n" << dumpDeps(deps);
    EXPECT_TRUE(hasB) << "Missing Content dep for fileB\n" << dumpDeps(deps);
}

TEST_F(ReplayDepPrecisionTest, Replay_SharedFile_NotDoubleCounted)
{
    ...
    size_t count = countDeps(deps, CanonicalQueryKind::FileBytes, sharedFile.path.filename().string());
    EXPECT_EQ(count, 1u)
