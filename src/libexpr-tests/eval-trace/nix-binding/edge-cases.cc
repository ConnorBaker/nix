...
TEST_F(NixBindingEdgeCaseTest, NixBinding_HyphenatedName_Tracked)
{
    ...
    EXPECT_TRUE(hasJsonDep(deps, CanonicalQueryKind::StructuredProjection, nixBindingPred("my-package")))
...
TEST_F(NixBindingEdgeCaseTest, NixBinding_QuotedName_Tracked)
{
    TempTestFile file("{ \"hello world\" = 1; y = 2; }");
    auto expr = std::format("(import {}).\"hello world\"", file.path.string());

    auto deps = evalAndCollectDeps(expr);
    EXPECT_TRUE(hasJsonDep(deps, CanonicalQueryKind::StructuredProjection, nixBindingPred("hello world")))
...
TEST_F(NixBindingEdgeCaseTest, NixBinding_InheritPattern_Tracked)
{
...
TEST_F(NixBindingEdgeCaseTest, NixBinding_InheritFromExpr_Tracked)
{
    ...
    EXPECT_TRUE(hasJsonDep(deps, CanonicalQueryKind::StructuredProjection, nixBindingPred("toString")))
...
TEST_F(NixBindingEdgeCaseTest, NixBinding_EmptyAttrset_NoDeps)
{
    TempTestFile file("{ }");
...
TEST_F(NixBindingEdgeCaseTest, NixBinding_NestedAttrset_TopLevelOnly)
{
    ...
    EXPECT_TRUE(hasJsonDep(deps, CanonicalQueryKind::StructuredProjection, nixBindingPred("outer")))
        << "Top-level binding should get NixBinding dep\n" << dumpDeps(deps);
    EXPECT_FALSE(hasJsonDep(deps, CanonicalQueryKind::StructuredProjection, nixBindingPred("inner")))
...
TEST_F(NixBindingEdgeCaseTest, NixBinding_AssertBody_NoTracking)
{
...
TEST_F(NixBindingEdgeCaseTest, NixBinding_DuplicateAccess_SingleDep)
{
    TempTestFile file("{ x = 1; y = 2; }");
    auto expr = std::format("let f = import {}; in f.x + f.x + f.x", file.path.string());

    auto deps = evalAndCollectDeps(expr);
    auto count = countJsonDeps(deps, CanonicalQueryKind::StructuredProjection, nixBindingPred("x"));
    EXPECT_EQ(count, 1u)
