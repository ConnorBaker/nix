...
protected:
    ...
    {
        return countJsonDeps(deps, CanonicalQueryKind::StructuredProjection,
...
TEST_F(NixBindingBasicTest, NixBinding_SingleAccess_RecordsDep)
{
    TempTestFile file("{ x = 1; y = 2; }");
    auto expr = std::format("(import {}).x", file.path.string());

    auto deps = evalAndCollectDeps(expr);
    EXPECT_TRUE(hasJsonDep(deps, CanonicalQueryKind::StructuredProjection, nixBindingPred("x")))
        << "Accessing .x should record a NixBinding SC dep\n" << dumpDeps(deps);
}

TEST_F(NixBindingBasicTest, NixBinding_UnaccessedBinding_NoDep)
{
    TempTestFile file("{ x = 1; y = 2; }");
    auto expr = std::format("(import {}).x", file.path.string());

    auto deps = evalAndCollectDeps(expr);
    EXPECT_FALSE(hasJsonDep(deps, CanonicalQueryKind::StructuredProjection, nixBindingPred("y")))
...
TEST_F(NixBindingBasicTest, NixBinding_MultiAccess_EachBindingRecorded)
{
    TempTestFile file("{ a = 1; b = 2; c = 3; d = 4; }");
    auto expr = std::format("let f = import {}; in f.a + f.b + f.c", file.path.string());

    auto deps = evalAndCollectDeps(expr);
    EXPECT_TRUE(hasJsonDep(deps, CanonicalQueryKind::StructuredProjection, nixBindingPred("a")))
        << dumpDeps(deps);
    EXPECT_TRUE(hasJsonDep(deps, CanonicalQueryKind::StructuredProjection, nixBindingPred("b")))
        << dumpDeps(deps);
    EXPECT_TRUE(hasJsonDep(deps, CanonicalQueryKind::StructuredProjection, nixBindingPred("c")))
        << dumpDeps(deps);
    EXPECT_FALSE(hasJsonDep(deps, CanonicalQueryKind::StructuredProjection, nixBindingPred("d")))
...
TEST_F(NixBindingBasicTest, NixBinding_HasAttr_RecordsDep)
{
    TempTestFile file("{ x = 1; y = 2; }");
    auto expr = std::format("(import {}) ? x", file.path.string());
...
TEST_F(NixBindingBasicTest, NixBinding_GetAttr_RecordsDep)
{
    TempTestFile file("{ x = 1; y = 2; }");
...
TEST_F(NixBindingBasicTest, NixBinding_RecAttrset_NoTracking)
{
...
TEST_F(NixBindingBasicTest, NixBinding_DynamicAttrs_NoTracking)
{
...
TEST_F(NixBindingBasicTest, NixBinding_ListReturn_NoTracking)
{
...
TEST_F(NixBindingBasicTest, NixBinding_IfReturn_NoTracking)
{
...
TEST_F(NixBindingBasicTest, NixBinding_LambdaWrapped_RecordsDep)
{
...
TEST_F(NixBindingBasicTest, NixBinding_LetWrapped_RecordsDep)
{
...
TEST_F(NixBindingBasicTest, NixBinding_WithWrapped_RecordsDep)
{
...
TEST_F(NixBindingBasicTest, NixBinding_ContentDep_AlwaysPresent)
{
    TempTestFile file("{ x = 1; y = 2; }");
    auto expr = std::format("(import {}).x", file.path.string());

    auto deps = evalAndCollectDeps(expr);
    EXPECT_TRUE(hasDep(deps, CanonicalQueryKind::FileBytes, file.path.string()))
