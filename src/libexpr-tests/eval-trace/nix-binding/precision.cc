...
TEST_F(NixBindingPrecisionTest, NixBinding_CommentChange_CacheHit)
{
...
TEST_F(NixBindingPrecisionTest, NixBinding_WhitespaceChange_CacheHit)
{
    TempTestFile file("{ x = 1; y = 2; }");
    auto expr = std::format("(import {}).x", file.path.string());
...
TEST_F(NixBindingPrecisionTest, NixBinding_AddUnrelatedBinding_CacheHit)
{
    TempTestFile file("{ x = 1; y = 2; }");
    auto expr = std::format("(import {}).x", file.path.string());
...
TEST_F(NixBindingPrecisionTest, NixBinding_ModifyUnaccessed_CacheHit)
{
    TempTestFile file("{ x = 1; y = 2; }");
    auto expr = std::format("(import {}).x", file.path.string());
...
TEST_F(NixBindingPrecisionTest, NixBinding_ModifyAccessedBinding_CacheMiss)
{
    TempTestFile file("{ x = 1; y = 2; }");
    auto expr = std::format("(import {}).x", file.path.string());
...
TEST_F(NixBindingPrecisionTest, NixBinding_RemoveAccessedBinding_CacheMiss)
{
    TempTestFile file("{ x = 1; y = 2; }");
    auto expr = std::format("(import {}).x", file.path.string());
    ...
    try {
        ...
        // Expected: .x doesn't exist anymore — Nix throws nix::Error for
        // attribute-not-found. We only catch nix::Error to avoid swallowing
        // unexpected exceptions (e.g., std::bad_alloc, test infrastructure failures).
    }
