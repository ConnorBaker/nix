...
| `store/dep-flattening-baseline.cc` | `MaterializationDepTest` | The 607× root cause pinned at unit scale (2 tests): `{cx=d.x; cy=d.y;}` over a shared JSON file → BOTH `cx` and `cy` stored traces independently carry a FileBytes dep on the shared file (3/3 in the scaling test). Non-vacuity: cx's trace carries `$["x"]` StructuredProjection, cy's `$["y"]` → distinct per-sibling traces. The regression-guarded "before" a successful edge change must visibly improve. (Distinct from the same-prefixed `ReplayIntegrationTest.SharedFileDep_*`) |
