...
| `store/derivation-input-flattening.cc` | `MaterializationDepTest` | Real-eval proof that the SPA dep is NOT the input-identity edge (3 tests): a derivation whose `args` embed `readFile(shared)`, consumed by two siblings via `.outPath`, flattens the shared `FileBytes` into BOTH consumer traces (607× is REAL for derivations) ALONGSIDE an inert `StorePathAvailability(.drv)` dep; `DrvInputFile_Change_InvalidatesOutPathConsumer` confirms the FileBytes carries soundness. `DrvInput_FlattenSite_ForceVsOutputRead` (follow-up #7→#8) is the FLATTEN-SITE discriminator, confound-split: cx=`d.outPath`/cy=`d.drvPath` (force args + read output), cw=`deepSeq (removeAttrs d [outputs])` (forces args DEEPLY, NO output read), cz=`attrNames d`/cv=`seq d` (do NOT force args). MEASURED cx=cy=cw=1, cz=cv=0. The decisive case cw (FileBytes=1 with no output read) shows the flatten is FORCE-OF-ARGS-driven, NOT output-read-driven — REFUTING follow-up #7's first cut (which used only cz and was confounded: attrNames varies both "no output read" AND "doesn't force args"). Consequence: a `coerceToContextObject` edge at the output read is ADDITIVE not flatten-replacing (fires after `derivationStrict`'s forceAttrs already recorded FileBytes); the only flatten-replacing site is a sub-scope at args-force = the reverted aggressive shape. See redesign-plan follow-up #8. Consequence: the RFC §3 producer edge must be a producer-TRACE-HASH edge folding in input deps, not the SPA dep. Backs the RFC's "MEASURED CORRECTION" block |
