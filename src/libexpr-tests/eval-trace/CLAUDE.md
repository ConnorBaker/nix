...
| `store/derivation-edge-soundness.cc` | `TraceStoreTest` | Tier-1 derivation-edge SOUNDNESS FLOOR (5 synthetic tests, mirror keyset-escape.cc) for the content-addressed-trace-identity RFC. Consumer A → producer B edge via `TraceValueContext` (B's trace hash = output identity): C1 producer source v1→v2 invalidates A; C2 unchanged B → A hits storing ONE edge not a closure copy; C2b shared producer verified once/session+memoized vs distinct producers re-walked (amortization, sharedLater=0 vs distinctLater=4); C3 ADVERSARIAL facet-observing consumer with output-only edge STALE-SERVES on a non-output facet change (pins WHY Tier-1 is gated on output-only observation); C3-corollary output-only consumer not over-invalidated. The edge does NOT let A skip B's closure — `resolveTraceContextHash` recursively verifies B; the win is 1 edge + once/session memoization (build-layer `drvHashes` amortization lifted to eval) |
| `store/derivation-observation-facets.cc` | `DepPrecisionTest` | The Tier-1 output-only GATE precondition, measured on real derivations via `evalAndCollectDeps` (3 tests): `(drv).outPath` records ONLY `StorePathAvailability(.drv)` + `sessionSystemValue` — no FileBytes/StructProj (output-only ⇒ edge drops nothing ⇒ sound); real `mkDerivation` shape `(drv // {meta=…;}).outPath` does NOT force meta (meta lives on the OUTER attrset, not a `derivationStrict` arg); a file-backed facet read (`meta=fromJSON(readFile)`) DOES record FileBytes+StructProj + no `.drv` dep → the facet hazard is real AND the gate "emit edge only when no facet dep recorded" is mechanically checkable from the dep set |
| `store/dep-flattening-baseline.cc` | `MaterializationDepTest` | The 607× root cause pinned at unit scale (3 tests): `{cx=d.x; cy=d.y;}` over a shared JSON file → BOTH `cx` and `cy` stored traces independently carry a FileBytes dep on the shared file (3/3 in the scaling test). Non-vacuity: cx's trace carries `$["x"]` StructuredProjection, cy's `$["y"]` → distinct per-sibling traces. `SharedDrv_SameOutPath_BothConsumersFlattenInputClosure` (2026-05-31) is the DECISIVE consumer-sharing test for the producer-partition RFC: a SHARED derivation `d` consumed by two siblings BOTH reading the SAME `d.outPath` (d forced once, memoized) — MEASURED cxHasFile=cyHasFile=1, so a shared derivation's input closure flattens into EVERY consumer via `replayMemoizedRange`, not just the first forcer. Confirms the RFC §7.7b sharing premise at unit scale and REPLACES the retracted `drv-sharing-probe` (which was memoization-confounded — see redesign-plan follow-up #9 retraction / #10 correction; lesson: validate a probe against a controlled case before trusting its verdict, and prefer inspecting the stored artifact over instrumenting a guessed event). The regression-guarded "before" a successful edge change must visibly improve. (Distinct from the same-prefixed `ReplayIntegrationTest.SharedFileDep_*`) |
| `store/derivation-outpath-soundness.cc` | `TraceStoreTest` + `TraceCacheFixture` | The make-or-break Tier-1 worry, resolved favorably (3 tests): `StorePathAvailability(.drv)` verifies EXISTENCE only — could an input change (→new `.drv`) leave the OLD trace verifying (old `.drv` still valid) and stale-serve? MEASURED NO. `DrvPath_IsContentAddressedByInputs` (arg v1 vs v2 → different `.drv` keys); `DrvOutPath_InputChange_NoStaleServe` (end-to-end real cache+store, file-backed input v1→v2 → warm serves NEW outPath, loaderCalls=1: caught by INPUT deps, not bypassed by the existence check); `DrvOutPath_InputUnchanged_WarmHit` (calls=0). An output-identity edge is sound because input changes invalidate via input observations |
| `store/derivation-input-flattening.cc` | `MaterializationDepTest` | Real-eval proof that the SPA dep is NOT the input-identity edge (4 tests): a derivation whose `args` embed `readFile(shared)`, consumed by two siblings via `.outPath`, flattens the shared `FileBytes` into BOTH consumer traces (607× is REAL for derivations) ALONGSIDE an inert `StorePathAvailability(.drv)` dep; `DrvInputFile_Change_InvalidatesOutPathConsumer` confirms the FileBytes carries soundness. `DrvInput_FlattenSite_ForceVsOutputRead` (follow-up #7→#8) is the FLATTEN-SITE discriminator, confound-split: cx=`d.outPath`/cy=`d.drvPath` (force args + read output), cw=`deepSeq (removeAttrs d [outputs])` (forces args DEEPLY, NO output read), cz=`attrNames d`/cv=`seq d` (do NOT force args). MEASURED cx=cy=cw=1, cz=cv=0. The decisive case cw (FileBytes=1 with no output read) shows the flatten is FORCE-OF-ARGS-driven, NOT output-read-driven — REFUTING follow-up #7's first cut (which used only cz and was confounded: attrNames varies both "no output read" AND "doesn't force args"). Consequence: a `coerceToContextObject` edge at the output read is ADDITIVE not flatten-replacing (fires after `derivationStrict`'s forceAttrs already recorded FileBytes); the only flatten-replacing site is a sub-scope at args-force = the reverted aggressive shape. See redesign-plan follow-up #8. `DrvIgnoreNullsDroppedRead_ChangeInvalidatesConsumer` is the `derivation-producer-partition-rfc.md` §2/§7.1 OBLIGATION PIN: `__ignoreNulls=true` (nixpkgs default) makes `derivationStrict` force-then-drop a null `extra` attr (primops.cc:1795-1798), so a file-backed dropped attr records a readFile dep that does NOT fold into drvPath — `cond.txt` "aaa"→"bbb" keeps drvPath identical yet the consumer must invalidate (`calls==1`). Pins the conservative shape's soundness for this case TODAY and is the red obligation any producer-isolation prototype (which would key by drvPath) must keep green. Consequence (SPA test): the RFC §3 producer edge must be a producer-TRACE-HASH edge folding in input deps, not the SPA dep. Backs the "MEASURED CORRECTION" block of `content-addressed-trace-identity-rfc.md` |
| `store/derivation-edge-loadbearing.cc` | `MaterializationDepTest` | Decision-critical characterization probe (`DrvOutPath_OldDrvValidityAfterInputChange`): records `d.outPath`, extracts the recorded `.drv` path, mutates the input, and queries `isValidPath(oldDrv)` before/after — the logged verdict (old `.drv` stays valid ⇒ SPA-alone would stale-serve ⇒ flattened FileBytes is load-bearing ⇒ RFC edge must be a producer-trace-hash) is the payload; the hard-enforced assertion is the weaker "`.drv` recorded + parseable." Settles the SPA-inertness question on the real evaluator |
| `store/ca-trace-key-routing.cc` | `TraceStoreTest` | RFC §3a routing-key feasibility, proven against the real store/vocab/verify pipeline with zero production change (3 tests). Design A = synthetic `"__ca:<drvHash>"` interned via `internName`: R1 a CA-keyed producer trace round-trips + verifies (first-class identity); R2 TWO consumers at genuinely DIFFERENT attr-path positions both edge to the SAME CA producer via `TraceValueContext` and BOTH hit (the cross-scope sharing C2b couldn't show with a literal vpath); R3 mutating the shared producer's input invalidates BOTH. Load-bearing proven by probe-then-revert (corrupting the stored producer hash flips R2 to a miss). The consume-side floor for the RFC; only §3b (producer boundary) is net-new |
| `store/ca-producer-boundary-recording.cc` | `TraceStoreTest` | RFC §3b RECORDING-side floor: drives the real `DepRecordingContext` via `TestScopeAccess` to prove the COMPOSITION (capture inputs in nested scope → finalize as CA producer → emit edge in parent) works through the existing scope + store machinery with zero production change (6 tests, all probe-verified). P1 scope-isolated producer + parent-edges-not-flattens; P2 consumer dep count INDEPENDENT of producer closure size (the 607×→1 collapse, M=2 vs M=50); P3 producer-input change invalidates consumer via the edge; P4 NESTED producers (depth-2 chain C→A→B), mutation at deepest level propagates up entire edge chain (the build-layer `hashDerivationModulo` recursion lifted to eval); P5 idempotency — duplicate `ctx.record(edge)` dedupes to one stored dep; P6 sketches the producer-side-table interface (`Value*→{caKey, traceHash}`) the §3b recorder needs to route between edge-emission (producer) and flatten-replay (non-producer) at force time |
| `dep/producer-side-table.cc` | `ProducerSideTableTest` (EvalTraceTest) | RFC §3b SIDE-TABLE API: pins the smallest production surface needed to support the gate — `MemoReplayStore::registerProducer/lookupProducer/clearProducerMap/producerMapSize` + `producerBloom` (added 2026-05-31). 5 tests, all probe-verified: T1 register+lookup round-trip; T2 distinct Values map to distinct CA keys; T3 re-registration overwrites stale entry (BUG-7 GC-address-reuse hazard); T4 unregistered Value returns nullopt; T5 clearProducerMap empties. Tests use the `_ForTest` synthetic-pointer fallback (`testProducerKey` reinterprets non-attrset Values' addresses); production keys by `const Bindings *` (see `dep/replay-producer-gate.cc` for the rationale) |
| `dep/replay-producer-gate.cc` | `ReplayProducerGateTest` (EvalTraceTest) | RFC §3b HOT-PATH GATE: pins the routing decision in `TraceRuntime::replayMemoizedDeps` between flatten-replay and edge-emission. 4 tests, all probe-verified: G1 registered Value emits edge (not flattened deps); G2 unregistered still flattens (control); G3 no recording scope ⇒ no-op; G4 producer with NO epoch range ⇒ edge still emitted. The gate fires BEFORE `getReplayRange` so a producer whose deps were already consumed via outer takeDeps still routes correctly. Adversarial: gate-disabled catches G1+G4; gate-fires-for-all-Values catches G1+G2+G4. Production gate uses `producerKeyFor(v) = v.attrs() if nAttrs else nullptr` (Bindings* keying — Value* is unstable across `vRes = vCur` copy in callFunction); these tests use the `_ForTest` synthetic-pointer fallback to exercise the gate with `mkInt`-shaped Values |
| `dep/ca-producer-scope.cc` | `TraceStoreTest` | RFC §3b END-TO-END chain through real `state.traceCtx` (3 tests, all probe-verified). S1 pins the AGGRESSIVE-shape composition (sub-scope isolation + edge-instead-of-flatten) — RETAINED as a regression guard for the recording-side primitives, NOT a pin on production behavior (production uses the conservative shape: epoch-range snapshot, no isolation; see header comment). S2 BoundValue_ReplayGateEmitsEdge: a registered Value re-forced via `replayMemoizedDeps` automatically emits one TraceValueContext edge. S3 ProducerInputMutation_InvalidatesConsumerViaGate: end-to-end soundness via `resolveTraceContextHash`. Adversarial: gate-disabled catches S2+S3; mutation-skipped catches S3 |
| `dep/trace-session-record-ca-producer.cc` | `TraceSessionRecordCAProducerTest` (TraceCacheFixture) | RFC §3b PRODUCTION SYNC API: drives `TraceSession::recordCAProducer` end-to-end through a real `TraceSession` (cold recording path) — the entry point `prim_derivationStrict` calls. 4 tests, all probe-verified: R1 round-trip (persist + register); R2 distinct drvHashes → distinct CA AttrPathIds + trace hashes; R3 cross-session persistence (producer trace verifies after `releaseActiveSession()` + reopen); R4 input mutation invalidates the persisted producer. All Values are `mkAttrs(buildBindings(0).finish())` because production `recordCAProducer` requires nAttrs (extracts `Bindings *` for stable keying). Adversarial: stub-`recordSync`-returns-nullopt catches R1+R2+R3+R4; mutation-skipped catches R4 |
