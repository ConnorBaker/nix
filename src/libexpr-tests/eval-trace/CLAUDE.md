...
| `dep/producer-side-table.cc` | `ProducerSideTableTest` (EvalTraceTest) | RFC §3b SIDE-TABLE API: pins the smallest production surface needed to support the gate — `MemoReplayStore::registerProducer/lookupProducer/clearProducerMap/producerMapSize`. 5 tests, all probe-verified: T1 register+lookup round-trip; T2 distinct Values map to distinct CA keys (no collision); T3 re-registration overwrites stale entry (BUG-7 GC-address-reuse hazard, mirrors `recordThunkDeps`); T4 unregistered Value returns nullopt (the gate's fall-through); T5 clearProducerMap empties (lifecycle parity with `clearReplayIndex`). Map keyed by `const Value *` with `traceable_allocator` (same lifetime concerns as `epochMap`); INDEPENDENT of `epochMap` — a Value can be in producerMap without being in epochMap |
| `dep/replay-producer-gate.cc` | `ReplayProducerGateTest` (EvalTraceTest) | RFC §3b HOT-PATH GATE: pins the routing decision in `TraceRuntime::replayMemoizedDeps` between today's flatten-replay path and the new edge-emission path. 4 tests, all probe-verified: G1 a registered Value forced inside a recording scope ⇒ exactly one `TraceValueContext` edge in consumer scope, NOT 3 flattened content deps from the epoch range; G2 unregistered Value still flattens (CONTROL — the gate is selective by registration); G3 producer registered, no recording scope active ⇒ no-op; G4 producer registered with NO epoch range ⇒ edge still emitted (the gate is INDEPENDENT of the epoch map). The gate fires BEFORE `getReplayRange` so a producer whose deps were already consumed via outer takeDeps still routes correctly. Adversarial: gate-disabled (`if(false)`) catches G1+G4; gate-fires-for-all-Values catches G1+G2+G4 |
| `dep/ca-producer-scope.cc` | `TraceStoreTest` | RFC §3b END-TO-END chain through real `state.traceCtx`: composes the side-table API + the replay gate + `SqliteTraceStorage` to prove the full producer-boundary contract (3 tests, all probe-verified). S1 OpenFinalizeRegister_RoundTrips drives the §3b boundary shape inline (open producer scope, capture inputs, persist as CA producer trace, register Value*) and shows the consumer's outer scope holds own dep + 1 edge — NOT producer's flattened deps; producer trace verifies after a fresh DB session. S2 BoundValue_ReplayGateEmitsEdge: a registered Value re-forced via `replayMemoizedDeps` (production code path) AUTOMATICALLY emits one `TraceValueContext` edge — no manual `record(edge)`. S3 ProducerInputMutation_InvalidatesConsumerViaGate: full end-to-end soundness — consumer trace persisted with the gate-emitted edge dep correctly invalidates when producer input mutates, via `resolveTraceContextHash`. Adversarial: gate-disabled catches S2+S3; mutation-skipped catches S3 |
