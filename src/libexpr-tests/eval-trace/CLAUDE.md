...
| `store/derivation-input-flattening.cc` | `MaterializationDepTest` | Real-eval proof that the SPA dep is NOT the input-identity edge (2 tests): a derivation whose `args` embed `readFile(shared)`, consumed by two siblings via `.outPath`, flattens the shared `FileBytes` into BOTH consumer traces (607× is REAL for derivations) ALONGSIDE an inert `StorePathAvailability(.drv)` dep; `DrvInputFile_Change_InvalidatesOutPathConsumer` confirms the FileBytes carries soundness. Consequence: the RFC §3 producer edge must be a producer-TRACE-HASH edge folding in input deps, not the SPA dep. Backs the RFC's "MEASURED CORRECTION" block |
