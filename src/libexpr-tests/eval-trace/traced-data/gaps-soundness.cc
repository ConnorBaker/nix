...
TEST_F(TracedDataTest, TracedJSON_StandaloneStructuralDep_MarkerChanges)
{
    ...
    // child's trace-context dep fails → child also re-evaluated.
    // Confirms existing trace-context cascade behavior.
    TempJsonFile f(R"({"name":"foo","marker":"ok"})");
    ...
    // child's trace-context dep fails → child re-evaluated
    int loaderCalls = 0;
    {
        auto cache = makeCache(expr, &loaderCalls);
        auto root = forceRoot(*cache);
...
// ── Gap 2 (FIXED): genericClosure — explicit #len for iterated lists ─

TEST_F(TracedDataTest, TracedJSON_GenericClosure_StartSetGrows)
{
    // genericClosure observes startSet length while seeding the work set.
    // Cold: 2 nodes → closure has 2 elements.
    // Hot: 3 nodes, the startSet #len dep fails, so the cache cannot serve
    // the stale 2-element closure.
...
// ═══════════════════════════════════════════════════════════════════════
// Gap: ExprOpUpdate (//) with same-provenance disjoint subsets
//
// The // operator is a set UNION.  Two disjoint subsets of the same
// traced container (e.g. removeAttrs x ks1 // removeAttrs x ks2) can
// reunite into an output larger than either source.  This violates
// the shape-preserving requirement of DerivedContainerBuilder.
//
// Previously, ExprOpUpdate used DerivedContainerBuilder for all paths,
// causing a finishAttrs assertion failure (SIGABRT) in debug builds
// when the output was larger than the uniqueSourcePtr.
// ═══════════════════════════════════════════════════════════════════════

// Disjoint keys from the same JSON file merged via //.
// This is the exact pattern that triggered the SIGABRT crash.
TEST_F(TracedDataTest, TracedJSON_Update_SameProvenance_DisjointKeys)
{
    TempJsonFile file(R"({"left":{"a":1,"b":2},"right":{"c":3,"d":4}})");
    auto expr = R"(let j = builtins.fromJSON (builtins.readFile )" + file.path.string()
        + R"(); in (j.left // j.right).a)";

    {
        auto cache = makeCache(expr);
        auto v = forceRoot(*cache);
        // The // merges 4 keys (a,b,c,d) — larger than either source (2 each).
        // Must not crash.
        EXPECT_THAT(v, IsIntEq(1));
    }

    {
        ...
        auto cache = makeCache(expr, &loaderCalls);
        forceRoot(*cache);
        EXPECT_EQ(loaderCalls, 0); // cache hit — file unchanged
    }

    // Value of .a changes — should re-eval via scalar dep
    file.modify(R"({"left":{"a":99,"b":2},"right":{"c":3,"d":4}})");
    invalidateFileCache(file.path);
...
// Partially overlapping keys from the same JSON file.
// Output size is between max(lhs, rhs) and lhs + rhs.
TEST_F(TracedDataTest, TracedJSON_Update_SameProvenance_PartialOverlap)
{
    TempJsonFile file(R"({"left":{"a":1,"b":2,"c":3},"right":{"b":20,"c":30,"d":4}})");
    auto expr = R"(let j = builtins.fromJSON (builtins.readFile )" + file.path.string()
        + R"(); in j.left // j.right)";

    {
        auto cache = makeCache(expr);
        auto v = forceRoot(*cache);
        // a from left, b+c from right (override), d from right = 4 keys
        EXPECT_EQ(v.attrs()->size(), 4u);
    }
...
// Same-provenance disjoint merge through the layer optimization path.
// Small RHS (1 key) triggers the shouldLayer branch in ExprOpUpdate.
TEST_F(TracedDataTest, TracedJSON_Update_SameProvenance_DisjointKeys_LayerPath)
{
    TempJsonFile file(R"({"base":{"a":1,"b":2,"c":3},"extra":{"d":4}})");
    // RHS has 1 key — well under the default layer threshold (16)
    auto expr = R"(let j = builtins.fromJSON (builtins.readFile )" + file.path.string()
        + R"(); in j.base // j.extra)";

    {
        auto cache = makeCache(expr);
        auto v = forceRoot(*cache);
        EXPECT_EQ(v.attrs()->size(), 4u); // a,b,c,d — must not crash
    }
...
// Chained // reuniting three disjoint subsets of the same traced container.
TEST_F(TracedDataTest, TracedJSON_Update_SameProvenance_ChainedDisjoint)
{
    TempJsonFile file(R"({"p1":{"a":1},"p2":{"b":2},"p3":{"c":3}})");
    auto expr = R"(let j = builtins.fromJSON (builtins.readFile )" + file.path.string()
        + R"(); in j.p1 // j.p2 // j.p3)";

    {
        auto cache = makeCache(expr);
        auto v = forceRoot(*cache);
        EXPECT_EQ(v.attrs()->size(), 3u); // a,b,c — must not crash
    }
    ...
    // Add a key to p2 — cache must miss
    file.modify(R"({"p1":{"a":1},"p2":{"b":2,"x":99},"p3":{"c":3}})");
    invalidateFileCache(file.path);

    {
        ...
        auto v = forceRoot(*cache);
        EXPECT_EQ(loaderCalls, 1);
        EXPECT_EQ(v.attrs()->size(), 4u); // a,b,c,x
    }
}

// Scalar access through same-provenance disjoint merge — cache precision.
TEST_F(TracedDataTest, TracedJSON_Update_SameProvenance_ScalarAccess_CacheHit)
{
    TempJsonFile file(R"({"left":{"a":1},"right":{"b":2}})");
    auto expr = R"(let j = builtins.fromJSON (builtins.readFile )" + file.path.string()
        + R"(); in (j.left // j.right).a)";
    ...
    // Change unaccessed key — should be cache hit
    file.modify(R"({"left":{"a":1},"right":{"b":999}})");
    invalidateFileCache(file.path);

    {
        ...
        auto cache = makeCache(expr, &loaderCalls);
        auto v = forceRoot(*cache);
        EXPECT_EQ(loaderCalls, 0); // b changed but we only access .a
        EXPECT_THAT(v, IsIntEq(1));
    }
}

// ═══════════════════════════════════════════════════════════════════════
// Gap: filterSource / copyPathToStore on a single file (not a directory)
//
// builtins.filterSource can be called on a regular file (nixpkgs's
// copyPathToStore = builtins.filterSource (p: t: true) does this).
// The trace dep recording in addPath unconditionally called
// path.readDirectory() on the root to record a Directory dep, which
// crashed with "cannot read directory: Not a directory" for file paths.
// ═══════════════════════════════════════════════════════════════════════

TEST_F(TracedDataTest, FilterSource_OnFile_DoesNotCrash)
{
    TempTestFile file("hello world");
    auto expr = R"(builtins.filterSource (p: t: true) )" + file.path.string();

    {
        auto cache = makeCache(expr);
        auto v = forceRoot(*cache);
        // filterSource on a file produces a store path string — must not crash.
    ...
    // Modify the file — should re-evaluate
    file.modify("changed content");
    invalidateFileCache(file.path);
