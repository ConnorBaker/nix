...
TEST_F(TracedDataTest, JSON_LiteralString_EagerFallback)
{
...
TEST_F(TracedDataTest, JSON_DirectReadFile_StructuralDeps)
{
...
TEST_F(TracedDataTest, JSON_ModifiedString_EagerFallback)
{
    TempJsonFile file(R"({"x": 42})");
    // readFile result is concatenated with extra text, so the content hash
    // won't match the TextObject → eager fallback (no structural deps).
...
TEST_F(TracedDataTest, TOML_ScalarAccess_CacheHit)
{
...
TEST_F(TracedDataTest, TOML_UnusedKeyChange_CacheHit)
{
    TempTomlFile file("[section]\nused = \"stable\"\nunused = \"original\"\n");
...
TEST_F(TracedDataTest, TOML_UsedKeyChange_CacheMiss)
{
    TempTomlFile file("[section]\nname = \"hello\"\nextra = \"x\"\n");
    auto expr = R"(let t = builtins.fromTOML (builtins.readFile )" + file.path.string() + R"(); in t.section.name)";
...
TEST_F(TracedDataTest, JSON_MultipleAccess_UnusedKeyIgnored)
{
...
TEST_F(TracedDataTest, JSON_RootArray_UnusedElementIgnored)
{
...
TEST_F(TracedDataTest, JSON_DottedKey_DistinguishedFromNested)
{
...
TEST_F(TracedDataTest, JSON_BracketKey_CacheHit)
{
...
TEST_F(TracedDataTest, JSON_QuoteAndBackslashKey_CacheHit)
{
