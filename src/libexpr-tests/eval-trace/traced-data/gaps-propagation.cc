...
// concatLists does NOT use DerivedContainerBuilder because the output
// length (sum of all inputs) is not a shape property of any single
// source — see the doc comment on EvalState::concatLists in eval.cc.
//
// This test verifies that the warm cache serves the result without
// needing to re-read the source file.  When the alias path fires
// (single non-empty list), the output Value is a bitwise copy of the
// source, and ContainerProvenanceRegistry keys lists by heap-backed storage
// identity, so lookupTracedContainer on the alias finds the source's
// TracedContainerProvenance without
// re-registration.  The #len dep is recorded on the source list before
// the alias branch.  loaderCalls == 0 confirms no file I/O on warm path.
