...
TEST_F(TracedDataTest, JSON_ArrayShrinks_LengthDepFails)
{
...
TEST_F(TracedDataTest, JSON_ArrayBecomesEmpty_LengthDepFails)
{
...
TEST_F(TracedDataTest, JSON_ShapeDep_RerecordedAfterInvalidation)
{
...
TEST_F(TracedDataTest, JSON_HasAttrOnNestedObject_CacheHit)
{
...
TEST_F(TracedDataTest, JSON_MultipleContainers_ShapeDepsRecorded)
{
...
TEST_F(TracedDataTest, TOML_LengthAfterTypeChangeToTable_CacheMiss)
{
...
// ═══════════════════════════════════════════════════════════════════════
// TextObject map tests (multi-file, reuse, forced-before-fromJSON)
// ═══════════════════════════════════════════════════════════════════════

// ── Two files read before fromJSON ────────────────────────────────────

TEST_F(TracedDataTest, JSON_TwoFilesReadBefore_ProvExcluded)
{
...
TEST_F(TracedDataTest, JSON_SameReadFileTwoFromJSON_EachTracked)
{
...
TEST_F(TracedDataTest, JSON_ForcedBeforeFromJSON_ProvExcluded)
{
...
TEST_F(TracedDataTest, JSON_ToStringBeforeFromJSON_ProvExcluded)
{
    TempJsonFile file(R"({"x": "hello", "extra": "padding"})");
    auto expr = R"(let s = builtins.readFile )" + file.path.string()
        + R"(; in (builtins.fromJSON (builtins.toString s)).x)";
...
TEST_F(TracedDataTest, JSON_AppendContextBeforeFromJSON_ProvExcluded)
{
    TempJsonFile file(R"({"x": "hello", "extra": "padding"})");
    auto expr = R"(let s = builtins.readFile )" + file.path.string()
        + R"(; in (builtins.fromJSON (builtins.appendContext s {})).x)";
...
TEST_F(TracedDataTest, TOML_UnsafeDiscardContext_ProvExcluded)
{
    TempTomlFile file("[section]\nname = \"hello\"\nextra = \"padding\"\n");
    auto expr = R"(let s = builtins.readFile )" + file.path.string()
        + R"(; in (builtins.fromTOML (builtins.unsafeDiscardStringContext s)).section.name)";
