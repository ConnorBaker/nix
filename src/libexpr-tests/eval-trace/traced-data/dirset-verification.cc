...
TEST_F(DepPrecisionTest, DirSet_DepCountReduction_ManyDirs)
{
    ...
    size_t dirHasKeyDeps = countJsonDeps(deps, CanonicalQueryKind::StructuredProjection,
        [](const nlohmann::json & j) {
            return j.value("t", "") == "d" && j.contains("h") && j["h"] == "absent"
                && !j.contains("ds");
        });
    size_t aggregatedDeps = countJsonDeps(deps, CanonicalQueryKind::StructuredProjection,
...
TEST_F(MaterializationDepTest, DirSet_StoredDeps_ContainAggregateDep)
{
    ...
    for (auto & d : storedDeps) {
        if (d.type != CanonicalQueryKind::StructuredProjection) continue;
