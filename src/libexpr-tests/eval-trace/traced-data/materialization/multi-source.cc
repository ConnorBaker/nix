...
TEST_F(MaterializationDepTest, JsonTomlMerge_JsonKeyAdded_CacheMiss)
{
...

    {
        ...
        EXPECT_EQ(loaderCalls, 1) << "JSON key change → cache miss";
    }
}
...
TEST_F(MaterializationDepTest, JsonTomlMerge_TomlValueChanged_CacheHit)
{
...

    {
        ...
        EXPECT_EQ(loaderCalls, 0) << "TOML value change with same keys → cache hit";
    }
}
...
TEST_F(MaterializationDepTest, ReadDirThenJson_FileAdded_CacheMiss)
{
...

    {
        ...
        EXPECT_EQ(loaderCalls, 1) << "file addition → cache miss";
    }
}

// ── Two JSON files merged cross-scope ───────────────────────────────

TEST_F(MaterializationDepTest, TwoJsonMerge_TwoFiles_IndependentInvalidation)
{
...
TEST_F(MaterializationDepTest, JsonReadDirMerge_AttrNames_RecordsSCKeysPerOrigin)
{
