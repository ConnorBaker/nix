...
TEST_F(MaterializationDepTest, CacheHit_ShapeDepsPreserved_InStoredTrace)
{
    ...
    EXPECT_TRUE(hasJsonDep(deps1, CanonicalQueryKind::StructuredProjection, shapePred("keys")))
    ...
    EXPECT_TRUE(hasJsonDep(deps2, CanonicalQueryKind::StructuredProjection, shapePred("keys")))
        << "Second run: SC #keys should still be in stored trace\n" << dumpDeps(deps2);
}

TEST_F(MaterializationDepTest, CrossScopeCache_CacheHit_ReplayedDepsMatchFresh)
{
    TempJsonFile f(R"({"a":1,"b":2})");
    auto expr = std::format(
        "let d = {}; in {{ inherit d; names = builtins.attrNames d; }}",
    ...
    // Second evaluation — should hit cache
    {
        ...
        ASSERT_NE(namesAttr, nullptr);
        state.forceValue(*namesAttr->value, noPos);
        EXPECT_EQ(loaderCalls, 0) << "unchanged file → warm hit";
    }

    auto cachedDeps = getStoredDeps("names");

    EXPECT_TRUE(hasJsonDep(freshDeps, CanonicalQueryKind::StructuredProjection, shapePred("keys")))
        << "Fresh deps should have SC #keys\n" << dumpDeps(freshDeps);
    EXPECT_TRUE(hasJsonDep(cachedDeps, CanonicalQueryKind::StructuredProjection, shapePred("keys")))
        << "Cached deps should have SC #keys\n" << dumpDeps(cachedDeps);
}

TEST_F(MaterializationDepTest, CrossScopeCache_MultiChild_IndependentInvalidation)
{
    TempJsonFile f(R"({"a":1,"b":2})");
    auto expr = std::format(
    ...
    EXPECT_EQ(lt->entries.size(), 3u) << "names should have 3 elements after key addition";
...
TEST_F(MaterializationDepTest, CrossScopeCache_StringList_OnlyForcedElementsStored)
{
