...
TEST_F(DepPrecisionCreationTest, JSON_Object_NoImplicitShapeLenDep)
{
    TempJsonFile file(R"({"x": 1, "y": 2})");
    auto expr = std::format("({}).x", fj(file.path));

    auto deps = evalAndCollectDeps(expr);
    EXPECT_FALSE(hasJsonDep(deps, CanonicalQueryKind::ImplicitStructure, shapePred("len")))
...
TEST_F(DepPrecisionCreationTest, Array_ImplicitShapeLen_Present)
{
    ...
    auto deps = evalAndCollectDeps(expr);
    EXPECT_TRUE(hasJsonDep(deps, CanonicalQueryKind::ImplicitStructure, shapePred("len")))
        << "Array creation must record ImplicitShape #len\n" << dumpDeps(deps);
}

TEST_F(DepPrecisionCreationTest, JSON_Array_NoImplicitShapeKeysDep)
{
    TempJsonFile file(R"({"items": [1, 2, 3]})");
    auto expr = std::format("builtins.elemAt ({}).items 0", fj(file.path));
...
TEST_F(DepPrecisionCreationTest, JSON_RootArray_ImplicitShapeLenPresent)
{
...
TEST_F(DepPrecisionCreationTest, Scalar_String_RecordsSCDep)
{
    ...
    auto deps = evalAndCollectDeps(expr);
    EXPECT_TRUE(hasDep(deps, CanonicalQueryKind::StructuredProjection, "name"))
...
TEST_F(DepPrecisionCreationTest, Scalar_Bool_RecordsSCDep)
{
    ...
    auto deps = evalAndCollectDeps(expr);
    EXPECT_TRUE(hasDep(deps, CanonicalQueryKind::StructuredProjection, "enabled"))
...
TEST_F(DepPrecisionCreationTest, Scalar_Null_RecordsSCDep)
{
    ...
    auto deps = evalAndCollectDeps(expr);
    EXPECT_TRUE(hasDep(deps, CanonicalQueryKind::StructuredProjection, "data"))
...
TEST_F(DepPrecisionCreationTest, Scalar_Float_RecordsSCDep)
{
    ...
    auto deps = evalAndCollectDeps(expr);
    EXPECT_TRUE(hasDep(deps, CanonicalQueryKind::StructuredProjection, "pi"))
...
TEST_F(DepPrecisionCreationTest, Scalar_NestedAccess_RecordsSCDep)
{
    ...
    auto deps = evalAndCollectDeps(expr);
    EXPECT_TRUE(hasDep(deps, CanonicalQueryKind::StructuredProjection, "value"))
...
TEST_F(DepPrecisionCreationTest, ReadDir_Creation_ImplicitShapeKeysPresent)
{
    TempDir dir;
...
TEST_F(DepPrecisionCreationTest, JSON_ContentDep_AlwaysPresentOnRead)
{
    TempJsonFile file(R"({"x": 1})");
    auto expr = std::format("({}).x", fj(file.path));

    auto deps = evalAndCollectDeps(expr);
    EXPECT_TRUE(hasDep(deps, CanonicalQueryKind::FileBytes, ""))
