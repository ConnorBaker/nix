...
TEST_F(FormalsComprehensiveTest, StrictFormals_PartialAccess_RecordsKeysAndAccessedField)
{
    ...
    EXPECT_TRUE(hasJsonDep(deps, CanonicalQueryKind::StructuredProjection, pathContainsPred(nlohmann::json({"a"}))))
        << "Field a accessed in body\n" << dumpDeps(deps);
    EXPECT_FALSE(hasJsonDep(deps, CanonicalQueryKind::StructuredProjection, pathContainsPred(nlohmann::json({"b"}))))
...
TEST_F(FormalsComprehensiveTest, Formals_StrictFormals_NoSCTypeDep)
{
    TempJsonFile file(R"({"a": 1, "b": 2})");
    auto expr = std::format("({{ a, b }}: a + b) ({})", fj(file.path));
...
TEST_F(FormalsComprehensiveTest, Formals_StrictFormals_NoSCHasKeyDep)
{
    TempJsonFile file(R"({"a": 1, "b": 2})");
    auto expr = std::format("({{ a, b }}: a + b) ({})", fj(file.path));
...
TEST_F(FormalsComprehensiveTest, StrictFormals_Merged_RecordsSCKeysPerOrigin)
{
    ...
    auto keysCount = countJsonDeps(deps, CanonicalQueryKind::StructuredProjection, shapePred("keys"));
...
TEST_F(FormalsComprehensiveTest, Formals_NestedStrict_OuterStrictInnerEllipsis)
{
...
TEST_F(FormalsComprehensiveTest, Formals_NestedEllipsis_PassThrough)
{
...
TEST_F(FormalsComprehensiveTest, Formals_StrictFormals_ResultPassedToAttrNames)
{
