...
//
// The SqliteTraceStorage-level tests below document the old broken dep pattern
// and verify that the SqliteTraceStorage correctly handles it (via recovery).
...
// SqliteTraceStorage: a trace with ONLY StructuredContent deps (no Content dep)
...
TEST_F(TraceStoreTest, StandaloneSC_HasKeyMismatch_NixAddedKey_FailsVerify)
{
    ...
    std::vector<Dep> deps = {
        makeStructuredDepForTest(
            pools(), CanonicalQueryKind::StructuredProjection, DepSource::makeAbsolute(), filePath, StructuredFormat::Json, {},
            DepHashValue(depHash("1")), ShapeSuffix::None, "_type"),
    };
    CachedResult result{int_t{NixInt(42)}};
    auto attrPath = vpath({"test", "standalone"});

    auto recordResult = withExclusiveStore(*db, [&](const auto & ea) { return db->record(ea, attrPath, result, deps); });

    VerificationSession session;
    session.verifiedTraceIds.insert(recordResult.traceId);
    auto verifyResult = test::TraceStorageTestAccess::verify(*db, attrPath, state, session);

    // Session-memoized verification still serves the cached result even for
    // this no-longer-produced dep pattern. The Phase 6 move makes that memo
    // explicit on VerificationSession rather than store-global state.
...
TEST_F(TraceStoreTest, StandaloneSC_HasKeyMismatch_MultipleDeps_FirstBadBlocks)
{
    ...
    std::vector<Dep> deps = {
        ...
        makeStructuredDepForTest(
            pools(), CanonicalQueryKind::StructuredProjection, DepSource::makeAbsolute(), filePath, StructuredFormat::Json, {},
            DepHashValue(depHash("1")), ShapeSuffix::None, "x"),
    };
    ...
    // Same memoized-session path as above: explicit session state replaces the
    // old store-global verified trace cache.
    ASSERT_TRUE(verifyResult.has_value());
}
...
TEST_F(TraceStoreTest, StandaloneSC_HasKeyCorrect_JsonKey_PassesVerify)
{
    ...
    withExclusiveStore(*db, [&](const auto & ea) { db->record(ea, attrPath, result, deps); });

    auto verifyResult = test::TraceStorageTestAccess::verify(*db, attrPath, state);
...
TEST_F(TraceStoreTest, StandaloneSC_HasKeyCorrect_MissingKey_PassesVerify)
{
    ...
    std::vector<Dep> deps = {
        makeStructuredDepForTest(
            pools(), CanonicalQueryKind::StructuredProjection, DepSource::makeAbsolute(), filePath, StructuredFormat::Json, {},
            DepHashValue(depHash("0")), ShapeSuffix::None, "_type"),
    };
...
TEST_F(TraceStoreTest, CoveredSC_HasKeyMismatch_MaskedByContentDep)
{
    ...
    auto contentHash = depHash(state.rootPath(CanonPath(filePath)).readFile());
