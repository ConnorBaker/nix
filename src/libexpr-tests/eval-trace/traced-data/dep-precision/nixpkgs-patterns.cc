...
TEST_F(DepPrecisionNixpkgsPatternsTest, NixpkgsPrecision_ConditionalPackageSet_CacheMiss)
{
...
TEST_F(DepPrecisionNixpkgsPatternsTest, NixpkgsPrecision_OverlayComposition_CacheHit)
{
    ...
    {
        auto deps = evalAndCollectDeps(expr);
        EXPECT_TRUE(hasDep(deps, CanonicalQueryKind::StructuredProjection, "pkg"))
...
TEST_F(DepPrecisionNixpkgsPatternsTest, MkDerivation_MetaFromJson_CacheMiss)
{
    ...
    {
        auto deps = evalAndCollectDeps(expr);
        EXPECT_TRUE(hasDep(deps, CanonicalQueryKind::StructuredProjection, "license"))
