...
TEST_F(DepPrecisionAttrKeysTest, AttrKeys_AttrNames_RecordsSCKeys)
{
...
TEST_F(DepPrecisionAttrKeysTest, AttrKeys_AttrNames_NoSCTypeDep)
{
    TempJsonFile file(R"({"x": 1, "y": 2})");
    auto expr = std::format("builtins.attrNames ({})", fj(file.path));
...
TEST_F(DepPrecisionAttrKeysTest, AttrKeys_AttrNames_NoSCLenDep)
{
    TempJsonFile file(R"({"x": 1, "y": 2})");
    auto expr = std::format("builtins.attrNames ({})", fj(file.path));
...
TEST_F(DepPrecisionAttrKeysTest, AttrKeys_AttrValues_RecordsSCKeys)
{
...
TEST_F(DepPrecisionAttrKeysTest, AttrKeys_AttrValues_NoSCTypeDep)
{
    TempJsonFile file(R"({"a": "x", "b": "y"})");
    auto expr = std::format("builtins.concatStringsSep \",\" (builtins.attrValues ({}))", fj(file.path));
...
TEST_F(DepPrecisionAttrKeysTest, AttrKeys_MapAttrsRemoveUnusedKey_CacheHit)
{
    ...
    {
        auto deps = evalAndCollectDeps(expr);
        EXPECT_TRUE(hasDep(deps, CanonicalQueryKind::StructuredProjection, "rev"))
            << dumpDeps(deps);
    }
