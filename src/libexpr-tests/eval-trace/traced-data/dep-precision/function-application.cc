...
TEST_F(DepPrecisionTest, Foldl_MergeAttrs_RecordsContent)
{
    ...
    EXPECT_TRUE(hasDep(deps, CanonicalQueryKind::FileBytes, f1.path.filename().string()))
        << "foldl' merge should record Content for file 1\n" << dumpDeps(deps);
    EXPECT_TRUE(hasDep(deps, CanonicalQueryKind::FileBytes, f2.path.filename().string()))
        << "foldl' merge should record Content for file 2\n" << dumpDeps(deps);
}

// ── Formal pattern matching on traced data ──────────────────────────

TEST_F(DepPrecisionTest, Formals_StrictFormals_RecordsSCKeys)
{
    TempJsonFile f(R"({"x":1,"y":2})");
    auto expr = std::format(
...
TEST_F(DepPrecisionTest, Formals_StrictFormals_NoSCTypeDep)
{
    TempJsonFile f(R"({"x":1,"y":2})");
    auto expr = std::format(
        "let apply = {{ x, y }}: x + y; in apply ({})", fj(f.path));
...
TEST_F(DepPrecisionTest, Formals_Default_RecordsSCKeysAndValue)
{
