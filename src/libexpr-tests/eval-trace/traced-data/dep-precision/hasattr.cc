...
TEST_F(DepPrecisionHasAttrTest, HasAttr_HasAttr_RecordsHasKey)
{
...
TEST_F(DepPrecisionHasAttrTest, HasKey_QuestionMark_Nested_KeyRemoved)
{
    ...
    // ── Dep verification ──
    {
        ...
        EXPECT_TRUE(hasJsonDep(deps, CanonicalQueryKind::StructuredProjection, hasKeyPred("y")))
...
TEST_F(DepPrecisionHasAttrTest, HasKey_QuestionMark_StringLiteralUnderscore_CacheHit)
{
    TempJsonFile file(R"({"_": 1})");
    auto expr = std::format(R"(({}) ? "_")", fj(file.path));

    {
        auto deps = evalAndCollectDeps(expr);
        EXPECT_TRUE(hasJsonDep(deps, CanonicalQueryKind::StructuredProjection, hasKeyPred("_")))
            << dumpDeps(deps);
    }
...
TEST_F(DepPrecisionHasAttrTest, HasKey_QuestionMark_StringLiteralUnderscore_KeyRemoved)
{
    TempJsonFile file(R"({"_": 1})");
    auto expr = std::format(R"(({}) ? "_")", fj(file.path));
...
TEST_F(DepPrecisionHasAttrTest, HasKey_QuestionMark_StringLiteralNullValue_CacheHit)
{
    TempJsonFile file(R"({"a": null})");
...
TEST_F(DepPrecisionHasAttrTest, HasKey_QuestionMark_StringLiteral_CrossCaseCacheHit)
{
    TempJsonFile fileA(R"({"x": 1})");
    TempJsonFile fileB(R"({"_": null})");
    auto exprA = std::format(R"(({}) ? "x")", fj(fileA.path));
    auto exprB = std::format(R"(({}) ? "_")", fj(fileB.path));

    testFingerprint = hashString(HashAlgorithm::SHA256, "dep-precision-hasattr-qmark-cross-a");
    {
    ...
    testFingerprint = hashString(HashAlgorithm::SHA256, "dep-precision-hasattr-qmark-cross-b");
    {
    ...
    {
        int loaderCalls = 0;
        auto cache = makeCache(exprB, &loaderCalls);
        auto v = forceRoot(*cache);
        EXPECT_EQ(loaderCalls, 0);
        EXPECT_THAT(v, IsTrue());
    }
...
TEST_F(DepPrecisionHasAttrTest, HasKey_SelectDefault_NestedPath_SiblingAdded)
{
    ...
    {
        auto deps = evalAndCollectDeps(expr);
        EXPECT_TRUE(hasJsonDep(deps, CanonicalQueryKind::StructuredProjection, pathContainsPred(nlohmann::json({"inner", "x"}))))
...
TEST_F(DepPrecisionHasAttrTest, HasAttr_HasOpNonAttrset_NonAttrsetIntermediate)
{
...
TEST_F(DepPrecisionHasAttrTest, HasKey_ReadDir_True_SiblingAdded)
{
    ...
    {
        auto deps = evalAndCollectDeps(expr);
        EXPECT_TRUE(hasJsonDep(deps, CanonicalQueryKind::StructuredProjection, hasKeyPred("foo")))
            << dumpDeps(deps);
    }
...
TEST_F(DepPrecisionHasAttrTest, HasKey_ReadDir_False_SiblingRemoved)
{
    ...
    {
        auto deps = evalAndCollectDeps(expr);
        EXPECT_TRUE(hasJsonDep(deps, CanonicalQueryKind::StructuredProjection, hasKeyPred("missing")))
            << dumpDeps(deps);
    }
...
TEST_F(DepPrecisionHasAttrTest, HasOp_KeyAdded_CacheMiss)
{
    ...
    {
        auto deps = evalAndCollectDeps(expr);
        EXPECT_TRUE(hasJsonDep(deps, CanonicalQueryKind::StructuredProjection, hasKeyPred("b")))
            << dumpDeps(deps);
    }
...
TEST_F(DepPrecisionHasAttrTest, SelectOrDefault_KeyPresent_CacheHit)
{
    ...
    {
        auto deps = evalAndCollectDeps(expr);
        EXPECT_TRUE(hasJsonDep(deps, CanonicalQueryKind::StructuredProjection, pathContainsPred(nlohmann::json({"b"}))))
            << "or-default with existing key records scalar dep for b\n" << dumpDeps(deps);
        EXPECT_TRUE(hasJsonDep(deps, CanonicalQueryKind::StructuredProjection, pathContainsPred(nlohmann::json({"a"}))))
            << "scalar dep for a\n" << dumpDeps(deps);
    }
