...
struct NixBindingCoverageFixture : TraceStoreFixture
{
    ...
    std::optional<NixBindingHash> computeRealBindingHash(
        ...
        const CachedResult & result)
    {
        ...
        withExclusiveStore(db, [&](const auto & ea) {
            db.record(ea, pathId, result, {
                makeContentDep(pools(), filePath.string(), fileContent),
                makeStructuredDepForTest(
                    pools(), CanonicalQueryKind::StructuredProjection, DepSource::makeAbsolute(), filePath.string(), StructuredFormat::Nix,
                    {StructuredPathComponent::makeKey(bindingName)}, DepHashValue(DepHash{bindingHash->value})),
            });
        });
        return true;
    }
