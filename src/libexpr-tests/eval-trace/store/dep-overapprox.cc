...
TEST_F(DepOverapproxFixture, Overapprox_UnusedFileDep_ContentFailsCausesSpuriousMiss)
{
    ...
    withExclusiveStore(*db, [&](const auto & ea) {
        db->record(ea, rootPath(), string_t{"result_from_a", {}}, {
...
TEST_F(DepOverapproxFixture, Overapprox_UsedFileDep_ContentFailsCorrectMiss)
{
    ...
    withExclusiveStore(*db, [&](const auto & ea) {
        db->record(ea, rootPath(), string_t{"result_from_main", {}}, {
            makeContentDep(pools(), mainFile.path.string(), "main_content_v1"),
            makeContentDep(pools(), aliasesFile.path.string(), "aliases_content_v1"),
        });
    });
...
TEST_F(DepOverapproxFixture, Overapprox_SCOnAccessedFile_UnusedFileStillCausesMiss)
{
    ...
    std::optional<NixBindingHash> bindingHash;
    ...
    withExclusiveStore(*db, [&](const auto & ea) {
        db->record(ea, rootPath(), string_t{"result_1", {}}, {
            makeContentDep(pools(), mainFile.path.string(), mainContent),
            makeStructuredDepForTest(
                pools(), CanonicalQueryKind::StructuredProjection, DepSource::makeAbsolute(), mainFile.path.string(), StructuredFormat::Nix,
                {StructuredPathComponent::makeKey("accessed")}, DepHashValue(DepHash{bindingHash->value})),
...
TEST_F(DepOverapproxFixture, Overapprox_SCOnBothFiles_UnusedFileChangeOverrideAccepts)
{
    ...
    auto makeScDep = [&](const std::filesystem::path & path,
        ...
        return makeStructuredDepForTest(
            pools(), CanonicalQueryKind::StructuredProjection, DepSource::makeAbsolute(), path.string(), StructuredFormat::Nix,
            {StructuredPathComponent::makeKey(bindingName)}, DepHashValue(DepHash{bh.value}));
    };
...
TEST_F(DepOverapproxFixture, Overapprox_SCOnBothFiles_UnrelatedBindingChange_OverrideAccepts)
{
