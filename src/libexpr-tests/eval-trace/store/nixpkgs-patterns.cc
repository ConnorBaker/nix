...
// Each test simulates a cache failure mode at the SqliteTraceStorage level.

// -- Category A: Irrelevant content change invalidates trace (aliases.nix pattern) --

TEST_F(TraceStoreTest, Miss_IrrelevantContentChange_AliasesPattern)
{
    ...
    withExclusiveStore(*db, [&](const auto & ea) {
        db->record(ea, vpath({"closures"}), string_t{"/nix/store/aaa-closures", {}}, deps);
    });
    ...
    auto result = test::TraceStorageTestAccess::verify(*db, vpath({"closures"}), state);
    EXPECT_FALSE(result.has_value())
        << "Expected cache miss: irrelevant dep changed but no prior trace "
           "with matching hash exists (aliases.nix pattern)";
}

TEST_F(TraceStoreTest, Hit_RevertAfterIrrelevantChange_DirectHashRecovery)
{
    ...
    withExclusiveStore(*db, [&](const auto & ea) {
        ...
        db->record(ea, vpath({"closures"}), string_t{"/nix/store/aaa-closures", {}}, depsV1);
        ...
        db->record(ea, vpath({"closures"}), string_t{"/nix/store/aaa-closures", {}}, depsV2);
    });
...
TEST_F(TraceStoreTest, Miss_NewDirectoryEntry_ChangesListing)
{
    ...
    withExclusiveStore(*db, [&](const auto & ea) {
        db->record(ea, vpath({"closures"}), string_t{"/nix/store/bbb-closures", {}}, deps);
    });
...
TEST_F(TraceStoreTest, Miss_IrrelevantLineAdded_SharedFilePattern)
{
    ...
    withExclusiveStore(*db, [&](const auto & ea) {
        db->record(ea, vpath({"closures"}), string_t{"/nix/store/ccc-closures", {}}, deps);
    });
...
TEST_F(TraceStoreTest, Miss_InterleavingGroups_NoMatchInHistory)
{
    ...
    withExclusiveStore(*db, [&](const auto & ea) {
        ...
        db->record(ea, vpath({"closures"}), string_t{"/nix/store/grp6", {}}, group6Deps);
        ...
        db->record(ea, vpath({"closures"}), string_t{"/nix/store/grp8", {}}, group8Deps);
    });
...
TEST_F(TraceStoreTest, Hit_InterleavingGroups_StructuralVariantRecovery)
{
    ...
    withExclusiveStore(*db, [&](const auto & ea) {
        ...
        db->record(ea, vpath({"closures"}), string_t{"/nix/store/v1", {}}, deps1);
        ...
        db->record(ea, vpath({"closures"}), string_t{"/nix/store/v2", {}}, deps2);
    });
...
TEST_F(TraceStoreTest, Miss_MultipleIrrelevantChanges_CompoundInvalidation)
{
    ...
    withExclusiveStore(*db, [&](const auto & ea) {
        db->record(ea, vpath({"closures"}), string_t{"/nix/store/multi", {}}, deps);
    });
...
TEST_F(TraceStoreTest, Hit_SameOutputDifferentHistory_DirectHashRecovery)
{
    ...
    withExclusiveStore(*db, [&](const auto & ea) {
        ...
        for (int i = 1; i <= 5; i++) {
            ...
            db->record(ea, vpath({"closures"}), string_t{"/nix/store/same-output", {}}, deps);
        }
    });
...
TEST_F(TraceStoreTest, Miss_NovelDepState_RecoveryFails)
{
    ...
    withExclusiveStore(*db, [&](const auto & ea) {
        ...
        db->record(ea, vpath({"closures"}), string_t{"/nix/store/same-output", {}}, depsV1);
        ...
        db->record(ea, vpath({"closures"}), string_t{"/nix/store/same-output", {}}, depsV2);
    });
