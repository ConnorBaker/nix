...
// ── ReplaceStringsReadFileGen ────────────────────────────────────────
//
// Generates: builtins.replaceStrings ["a"] ["b"] (builtins.readFile <file>)
// Kind::File, ResultKind::String.

rc::Gen<TestExpr> makeReplaceStringsReadFileGen()
{
    return rc::gen::map(
        ...
        [](std::string content) {
            ...
            std::string nixCode =
                "builtins.replaceStrings [\"a\"] [\"b\"] "
                "(builtins.readFile " + handle->path.string() + ")";
