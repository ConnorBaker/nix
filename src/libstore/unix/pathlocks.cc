...
bool lockFileWithTimeout(Descriptor desc, LockType lockType, unsigned int timeout)
{
    ...
    while (true) {
        ...
        auto actualSleep = std::min(sleepDuration,
...
AutoCloseFD acquireExclusiveFileLock(
    ...
    std::string_view identity)
{
    ...
    while (true) {
        ...
        if (st.st_ino != st_path.st_ino || st.st_dev != st_path.st_dev) {
            debug("lock file '%s' inode mismatch (fd: %lu, path: %lu), retrying",
                lockPath.string(), (unsigned long)st.st_ino, (unsigned long)st_path.st_ino);
            fd.close();
            continue;
        }
