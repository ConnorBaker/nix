...
# Eval cache dependency tracking tests: pathExists, readDir, flake input update,
# partial tree invalidation, derivation structure, path: flake caching.
...
###############################################################################
# Test 1: pathExists dependency tracking
###############################################################################
...
# optional.nix doesn't exist
nix build --no-link "$t1Dir#drv"
NIX_ALLOW_EVAL=0 nix build --no-link "$t1Dir#drv"

# Create optional.nix — pathExists dep should invalidate
...
echo "Test 1 passed: pathExists dependency tracking"

###############################################################################
# Test 2: readDir dependency tracking
###############################################################################
...
# Add another module — readDir dep should invalidate
...
echo "Test 2 passed: readDir dependency tracking"

###############################################################################
# Test 3: Flake input update
###############################################################################
...
# Update input (change dep-data.txt) but drv doesn't read from it
...
# drv depends only on local-data.txt — should still be cached
NIX_ALLOW_EVAL=0 nix build --no-link "$t3Dir#drv"

echo "Test 3 passed: flake input update"

###############################################################################
# Test 4: Partial tree invalidation (independent deps)
###############################################################################
...
# Modify only a-data.txt
echo "gamma" > "$t4Dir/a-data.txt"
git -C "$t4Dir" add a-data.txt
git -C "$t4Dir" commit -m "Modify a-data.txt"

# a should return updated value
[[ "$(nix eval --json "$t4Dir#a")" == '"gamma\n"' ]]

# b depends only on b-data.txt — should still be cached
[[ "$(NIX_ALLOW_EVAL=0 nix eval --json "$t4Dir#b")" == '"beta\n"' ]]

# After re-evaluation, both should be cached again
[[ "$(NIX_ALLOW_EVAL=0 nix eval --json "$t4Dir#a")" == '"gamma\n"' ]]
[[ "$(NIX_ALLOW_EVAL=0 nix eval --json "$t4Dir#b")" == '"beta\n"' ]]

echo "Test 4 passed: partial tree invalidation"

###############################################################################
# Test 5: nix show-derivation inspection (functional check)
###############################################################################
...
# Verify by re-reading from cache (functional check)
NIX_ALLOW_EVAL=0 nix eval "$t5Dir#result" | grepQuiet "test-content"

echo "Test 5 passed: derivation structure (cache functional)"

###############################################################################
# Test 6: path: flake caching
###############################################################################
...
# Should be cached on second run (path: flakes get stable identity)
NIX_ALLOW_EVAL=0 nix build --no-link "path:$t6Dir#drv"

# Modify the data file -- should invalidate
...
echo "Test 6 passed: path: flake caching"

echo "All eval-cache-deps tests passed!"
