...
# Both verify HIT — only description changed, outputs binding is identical.
# NixBinding SC deps override the Content dep failure (two-level verification).
NIX_ALLOW_EVAL=0 nix build --no-link "$t2Dir#drvA"
NIX_ALLOW_EVAL=0 nix build --no-link "$t2Dir#drvB"
...
# drvA: verify HIT — only description changed, outputs binding identical.
NIX_ALLOW_EVAL=0 nix build --no-link "$t4Dir#drvA"
...
# Warm the replay via NIX_ALLOW_EVAL=0 so this asserts "served from
# cache" rather than "re-evaluated and happened to produce the same
# error message." Without the guard, a regression that re-evaluated
# every call (but still produced the same throw) would pass silently.
expectStderr 1 env NIX_ALLOW_EVAL=0 nix build "$t7Dir#broken" \
  | grepQuiet 'error: intentional error'
...
# Capture NIX_SHOW_STATS to prove the non-cached IFD build actually
# records a fresh trace rather than replaying (evalTrace.record.count
# must grow). If the IFD-flag-change were silently cached, the
# counter would stay at its prior value.
...
NIX_SHOW_STATS=1 NIX_SHOW_STATS_PATH="$TEST_HOME/core-t7-ifd.json" \
  nix build --no-link "$t7Dir#ifd"
t7_ifd_records="$(readEvalTraceCounter "$TEST_HOME/core-t7-ifd.json" evalTrace.record.count)"
if [[ "$t7_ifd_records" -lt 1 ]]; then
  echo "Test 7 regression: IFD build did not record fresh trace (records=$t7_ifd_records)"
  cat "$TEST_HOME/core-t7-ifd.json" >&2
  exit 1
fi
...
# Normal depth should succeed — no stale error trace exists. Prove
# via NIX_SHOW_STATS that this build records a fresh trace
# (record.count > 0) rather than serving the stack-overflow from a
# cached error. If the SO were silently recorded and then replayed,
# this build would fail exit=1 AND record.count would be 0.
NIX_SHOW_STATS=1 NIX_SHOW_STATS_PATH="$TEST_HOME/core-t8-stack.json" \
  nix build --no-link "$t8Dir#stack-depth"
t8_records="$(readEvalTraceCounter "$TEST_HOME/core-t8-stack.json" evalTrace.record.count)"
if [[ "$t8_records" -lt 1 ]]; then
  echo "Test 8 regression: normal-depth build did not record a fresh trace (records=$t8_records)"
  echo "This would indicate the stack overflow was cached, then silently served."
  cat "$TEST_HOME/core-t8-stack.json" >&2
  exit 1
fi
