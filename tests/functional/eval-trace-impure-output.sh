...
# Switch to v2 with a visibly different value.
...
###############################################################################
# Test 13: Same-size file modification detected
# Verifies that modifying a file to same-length content is detected by the
# verification path, preventing stale traced results from passing verification.
###############################################################################
clearStoreIndex
...
# Should detect the change even though the byte length is unchanged.
...
echo "Test 13 passed: same-size modification detected for trace verification"

###############################################################################
# Test 14: Float precision preserved across trace cache (BUG-10 regression guard)
# std::to_string truncates to 6 significant digits; std::to_chars produces the
# shortest exact round-trip representation. This test verifies that a float
# with more than 6 significant digits round-trips correctly through the cache.
###############################################################################
clearStoreIndex

cat >"$testDir/test-float-precision.nix" <<'EOF'
3.141592653589793
EOF

# Cold evaluation — records trace with the full-precision float result.
result14_cold="$(nix eval --impure -f "$testDir/test-float-precision.nix")"

# Warm evaluation — served from cache.
result14_warm="$(nix eval --impure -f "$testDir/test-float-precision.nix")"

# Cold and warm must be identical — the cached value must not be truncated.
# Note: nix eval prints floats with Nix's showFloat (limited display precision),
# so the CLI output may show fewer digits than the internal double value.
# The critical property is cold == warm: the cache round-trip preserves the
# exact internal double, even if the display is truncated.  BUG-10 (std::to_string
# with 6 decimal places) would cause warm != cold because the INTERNAL value
# would be truncated, producing a different showFloat output.
[[ "$result14_cold" == "$result14_warm" ]] \
  || { echo "Float precision test FAILED: cold='$result14_cold' warm='$result14_warm'"; exit 1; }

echo "Test 14 passed: float precision preserved across trace cache (BUG-10)"
