...
76. **Closure-walk helpers (BFS over `references`).** `nix-store/dotgraph.cc` and `nix-store/graphml.cc` both implement the same `StorePathSet workList`/`doneSet` BFS over `references`. They differ only in edge-direction and per-node emission. A shared `walkClosure(start, visit)` helper would consolidate.
    ...
    - **Branch:** `vibe-coding/cleanup/nix-cli` (added a header-only template helper `walkClosure(store, roots, onPath, onEdge)` in a new `src/nix/nix-store/closure-walk.hh` — file-local to the `nix-store/` directory rather than promoted to `store-api.hh` per the validation's original sketch, because the latter would be a cross-shard libstore change. The helper takes per-path and per-edge callbacks so each caller emits its own format; queries `queryPathInfo` once per visited path and passes the `ValidPathInfo &` through (dotgraph ignores most fields, graphml uses them). Both consumer files now call `walkClosure(...)` with their own lambdas; node-emission and edge-direction divergence preserved. Output is byte-for-byte identical pre/post. **Unity-build follow-up (2026-05 audit):** the original landing (`705114e2c`) promoted `graphml.cc`'s local `makeEdge` lambda into a file-`static` function whose name and signature collided with `dotgraph.cc`'s pre-existing file-`static makeEdge`; this is fine under per-file compilation but is an ODR redefinition under the flake's *unity build*, which concatenates `dotgraph.cc` and `graphml.cc` into one translation unit. `nix build -L .` failed on it (per-file `ninja` does not exercise the unity build). Fixed by renaming the graphml helpers to `makeGraphMLEdge`/`makeGraphMLNode` (`makeNode` had the same latent collision — different signature, so it would have overloaded rather than redefined, but renamed for clarity); commit `fd60a040b`. `nix build -L .` green after the rename. Lesson recorded: re-verify *every* changed shard through the flake unity build, not just per-file `ninja`.)
...
255. **`parseInstallable` + `Installable::toStorePath` is open-coded at four sites in `why-depends.cc`.** `SourceExprCommand::parseInstallable(store, x)` followed by `Installable::toStorePath(getEvalStore(), store, Realise, operateOn, ...)` recurs four times. Surfaced by the 2026-05 audit (F036).
    ...
    - **Status (2026-05 foundation batch): declined after attempt.** Two corrections from implementation: (1) there are **two** `parseInstallable`+`toStorePath` pairs in `CmdWhyDepends::run` (for `_package` and `_dependency`), not four — the "four sites" count is wrong. (2) A `StorePath resolveToStorePath(...)` helper does NOT work: both `parseInstallable` results (`package`, `dependency`) are used again later — `printError("'%s' does not depend on '%s'", package->what(), dependency->what())` — so a helper that returns only the `StorePath` discards the `Installable` the error path needs. A `{Installable, StorePath}`-returning helper would save nothing over the two plain inline lines. Per the churn standard, left as-is. (Caught at build time: the over-extracted helper compiled the call sites but broke the later `->what()` uses.)
