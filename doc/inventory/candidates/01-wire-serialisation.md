...
1. **`BuildResult` serialiser duplicated across worker and serve protocols.** Both `WorkerProto::Serialise<BuildResult>` and `ServeProto::Serialise<BuildResult>` are near-byte-identical (same status / errorMsg / timing / builtOutputs sequence, only the version-cutoff literals differ — worker uses `{1,29}`/`{1,37}`/feature `realisation-with-path-not-hash`/`{1,28}`; serve uses `{2,3}`/`{2,8}`/`{2,6}`). Worker has an extra cpu-timing branch and a feature gate; serve does not. The shared `common = [&](errorMsg, isNonDeterministic, builtOutputs) { ... }` lambda is duplicated almost verbatim. The single biggest refactor target in the protocol layer.
   ...
   - **Branch:** `vibe-coding/cleanup/libstore` (lifted into `common-protocol-impl.hh` as `readBuildResult<Proto>` / `writeBuildResult<Proto>` function templates parameterised on the protocol class. Each per-protocol caller computes its own `hasV2Fields` / `hasNewBuiltOutputs` / `hasOldBuiltOutputs` predicates from its own version structure and passes them in as `bool`. The Worker-only cpu-timing branch is threaded as a callable hook, not a `bool`, because `WorkerProto` carries a `Serialise<std::optional<std::chrono::microseconds>>` specialisation that `ServeProto` lacks; a `bool` switch would force the templates to instantiate the missing specialisation. Wire format byte-for-byte unchanged. Landed together with #3.)
...
3. **`DrvOutput`/`UnkeyedRealisation`/`Realisation` serialisers identical between worker and serve.** Each pair is byte-identical except for the version-gate (`featureRealisationWithPath` for worker, `>= 2.8` for serve). Obvious candidate for promotion to `CommonProto` once a way to thread the per-protocol gate through is found.
   ...
   - **Branch:** `vibe-coding/cleanup/libstore` (lifted into `common-protocol-impl.hh` as `readDrvOutput<Proto>` / `readUnkeyedRealisation<Proto>` / `readRealisation<Proto>` function templates. The cross-protocol error semantics — Worker checks the `realisation-with-path-not-hash` feature; Serve checks `version >= {2, 8}` — stay at the per-protocol `Serialise<T>::read/write` sites because the messages and conditions differ. Landed together with #1.)
...
7. **Protocol handshake logic is parallel between worker and serve.** `WorkerProto::BasicClientConnection::handshake` and `ServeProto::BasicClientConnection::handshake` both send magic-1, read magic-2, exchange version numbers, take the min. Worker additionally exchanges and intersects a `FeatureSet` (≥1.38) via private `intersectFeatures`; serve has no such step. Server-side mirrors are likewise parallel.
   ...
   - **Branch:** `vibe-coding/cleanup/libstore` (added a `negotiateVersion<VersionT>(localVersion, peerVersion, localMin, tooOldMsg)` template helper in `common-protocol.hh` that does the bound-and-take-min computation client-side. Worker passes `Version::Number`, Serve passes a full `Version`; both have `<` and `std::min`. Server-side handshakes still call `std::min` directly because servers accept any client version and don't need the lower-bound check.)
