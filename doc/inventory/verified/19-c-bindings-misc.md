...
- `nix_err nix_store_path_hash(nix_c_context * context, const StorePath * store_path, nix_store_path_hash_part * hash_part_out)` — decode the path's hash from base32 into 20 raw bytes via `BaseNix32::decode`. **Outlier (2026-05 audit, F052):** this function does NOT clear `context->last_err_code` on entry, unlike every other fallible function in `nix_api_store.cc` — so a stale error from a prior call can survive a successful call and mislead a later `nix_err_code`. Same omission as `nix_fetchers_settings_new` (noted below); both are uncataloged drift sites. Minimal fix applied on `vibe-coding/cleanup/libstore` (add the `last_err_code = NIX_OK` preamble); the broader RAII rework is N29 / F038.
...
- `ExternalValue * nix_get_external(nix_c_context * context, nix_value * value)` — return `external()` cast to `ExternalValue *`. **BUG (2026-05 audit, F003):** this read-extractor uses `check_value_out` (which throws when the value *is* valid) whereas all 9 sibling read-extractors use `check_value_in`. Since the next line is `assert(v.type() == nix::nExternal)` (requires a valid external), the two checks are mutually exclusive — the function throws for *every* legitimate input, records the error, and returns `nullptr`. It ships via `install_headers` and has no in-tree caller, so CI never exercised it. Fix: `check_value_out` → `check_value_in` (applied on `vibe-coding/cleanup/libexpr`). The non-const-signature observation is secondary, not the headline.
