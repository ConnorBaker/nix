...
| [V2-e4-reachability.md](V2-e4-reachability.md) | Per-site reachability of E4's positional-format wire-crossing sites | **Counts substantially corrected**: positional `%N%` set is 71 in libstore (not 61), 50 in libutil; net wire-crossing positional ~95 (not 61). E4's "17 libutil throws" was a heavy undercount of all-format throws (281). E4 missed five categories: `store-api.hh` inline throws (load-bearing for the `"is not valid"` contract), `string2IntMustParse`, `TimedOut` ctor body, `%s`/`%d` non-positional wire-crossing, post-build `BuilderFailureError`. Phasing implication: positional and non-positional migrations must batch together for contract-bearing sites. Note: this report cites line numbers in places (against AGENT-CHARTER), useful for the verification but should not be propagated into catalog bodies |
