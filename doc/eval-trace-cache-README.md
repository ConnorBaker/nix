...
| **2** | **Semantic derivation-boundary transitive digest** (Bazel/Skyframe strict dependency capture + facet mask) | The genuinely-open frontier. Addresses the whole-file `FileBytes` rejection tail on by-name `package.nix` whose source changed but demanded output didn't (runs 87/88). The changed-package oracle was rejected (parent can depend on non-output attrs/passthru/meta). Needs a producer-side drvPath/output-facet proof, not a file whitelist. **No production build system does unobserved-member-changed reuse** — soundness burden is entirely ours. | redesign-plan §2026-05-29 CORRECTION finding 3; findings.md "Derivation Boundary Proof" |
