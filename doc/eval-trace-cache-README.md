...
| **2** | **Semantic derivation-boundary caching + transitive digest** (Bazel/Skyframe strict dependency capture + facet mask) | **The measured `closures.gnome` outlier fix (2026-05-30).** Outliers re-eval ~20 M thunks because the closure caches at ~7 coarse string leaves; the 6,419 derivations inside (99.66 % unchanged commit-to-commit) have no reuse boundary. Flamegraph: ~58 % of cost is `make-derivation`/pkgs (cacheable), ~25 % module fixpoint (floor). `derivationStrict` is currently uninstrumented for reuse (CLAUDE.md "outside scope"). Needs a facet mask (parent can read non-output attrs/passthru/meta) + compact per-derivation records (6,419/commit × fat blobs = v53 explosion). Sketch: `plans/lever2-derivation-boundary-caching.md`. | redesign-plan §2026-05-30 reframe; findings.md "Derivation Boundary Proof" |
