...
| **2** | **Semantic derivation-boundary caching** (Bazel/Skyframe strict dependency capture + facet mask) | **The measured `closures.gnome` outlier fix (2026-05-30), but RECLASSIFIED high-blast-radius by adversarial pass.** Outliers re-eval ~20 M thunks; the 6,419 derivations inside (99.66 % unchanged commit-to-commit) have no reuse boundary. Flamegraph: ~58 % cost cacheable (`make-derivation`/pkgs), ~25 % module fixpoint (floor). Adversarial finding (lever2 doc §7-9): per-thunk dep attribution already exists (epoch ranges), but the speedup needs verify-BEFORE-force, whose only hook is `TracedExpr::eval` — requiring `TracedExpr` wrappers installed at derivation thunk creation in the CORE EVALUATOR (hot path, all eval). Recording alone is inert. Recommended next step: a throwaway feasibility spike (does evaluator interception non-regress the 77 fast commits?), NOT a direct prototype. Plus facet mask + storage budget. Sketch: `plans/lever2-derivation-boundary-caching.md`. | redesign-plan §2026-05-30 reframe + adversarial; findings.md "Derivation Boundary Proof" |
