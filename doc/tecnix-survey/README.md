...
Tecnix's eleven `__unsafeTectonixInternal*` builtins are user-side compensation for three abstractions Nix already half-implements internally but doesn't expose: a tree-SHA → NAR-hash projection (used for tarballs but not arbitrary git subtrees), per-subpath mounting (used for git submodules but not language-visible), and overlays with whiteouts (re-implemented in tecnix's `DirtyOverlaySourceAccessor`). The same gaps surface in non-tecnix workloads: every `lib.cleanSource ./.` is uncached on disk ([§6.6](#66-filtered-sources-bypass-the-fetcher-cache--confirmed)); a subtree shared across N revs is hashed once per rev despite the underlying tree SHA being identical ([§6.1](#61-tree-sha-cache-miss-across-revisions--confirmed)); concurrent reads of one git repo serialise on a single mutex ([§7.5](#75-synchronisation-primitives-dont-form-a-coherent-menu)). The unifying claim of the document is **[§8.0](#80-the-natural-conclusion-finish-the-lazy-filesystem)**: `SourceAccessor` was always meant to be a lazy filesystem, and finishing it collapses all of these into a single construction.

Every claim is grounded in code read end-to-end[^codebases] or behaviour reproduced empirically.[^empirics] Limits of the analysis in [§9](#9-limits-of-this-analysis).
...
- [2. Setup: the layered model](#2-setup-the-layered-model)
  - [2.1 Three layers](#21-three-layers)
  - [2.2 The `SourceAccessor` algebra](#22-the-sourceaccessor-algebra)
  - [2.3 The L2 design split](#23-the-l2-design-split)
  - [2.4 The CLI always devirtualizes](#24-the-cli-always-devirtualizes)
  - [2.5 Where the caches and accessors live](#25-where-the-caches-and-accessors-live)
- [3. Tecnix patch set](#3-tecnix-patch-set)
  - [3.1 Eleven new builtins](#31-eleven-new-builtins)
  - [3.2 EvalState additions](#32-evalstate-additions)
  - [3.3 Settings](#33-settings)
  - [3.4 libfetchers/git extensions](#34-libfetchersgit-extensions)
  - [3.5 Other patches](#35-other-patches)
- [4. Per-patch mapping: what each tecnix patch is really asking for](#4-per-patch-mapping-what-each-tecnix-patch-is-really-asking-for)
- [5. Asymmetry catalogue](#5-asymmetry-catalogue)
  - [5.1 `getFingerprint` component composition — three different rules](#51-getfingerprint-component-composition--three-different-rules)
  - [5.2 `getProvenance` self-check — asymmetric](#52-getprovenance-self-check--asymmetric)
  - [5.3 `pathExists` override — asymmetric, **L**](#53-pathexists-override--asymmetric-l)
  - [5.4 `lstat` override — asymmetric](#54-lstat-override--asymmetric)
  - [5.5 `invalidateCache` — divergent across master and DetSys](#55-invalidatecache--divergent-across-master-and-detsys)
  - [5.6 `getLastModified` aggregation — absent](#56-getlastmodified-aggregation--absent)
  - [5.7 `showPath` shape — asymmetric](#57-showpath-shape--asymmetric)
  - [5.8 `mount` on duplicate key — silent drop](#58-mount-on-duplicate-key--silent-drop)
  - [5.9 `getProvenance` path translation — three different rules](#59-getprovenance-path-translation--three-different-rules)
  - [5.10 `ForwardingSourceAccessor` is incomplete (DetSys only)](#510-forwardingsourceaccessor-is-incomplete-detsys-only)
  - [5.11 `CachingSourceAccessor` (master only) is partial-forwarding](#511-cachingsourceaccessor-master-only-is-partial-forwarding)
  - [5.12 The L2 random-store-path hides materialisation state](#512-the-l2-random-store-path-hides-materialisation-state)
  - [Symmetric operations](#symmetric-operations)
- [6. Empirical evidence](#6-empirical-evidence)
  - [6.1 Tree-SHA cache miss across revisions — confirmed](#61-tree-sha-cache-miss-across-revisions--confirmed)
  - [6.2 String-interpolation hole — confirmed](#62-string-interpolation-hole--confirmed)
  - [6.3 Whiteout correctness in `DirtyOverlaySourceAccessor` — verified](#63-whiteout-correctness-in-dirtyoverlaysourceaccessor--verified)
  - [6.4 lazy-trees default behaviour — observed](#64-lazy-trees-default-behaviour--observed)
  - [6.5 `treeHashToNarHash` cache is alive in production](#65-treehashtonarhash-cache-is-alive-in-production)
  - [6.6 Filtered sources bypass the fetcher cache — confirmed](#66-filtered-sources-bypass-the-fetcher-cache--confirmed)
- [7. Larger patterns Nix is missing or duplicating](#7-larger-patterns-nix-is-missing-or-duplicating)
  - [7.1 The internal tree-SHA → NAR-hash projection that already exists, but isn't surfaced](#71-the-internal-tree-sha--nar-hash-projection-that-already-exists-but-isnt-surfaced)
  - [7.2 Subtree mounts already work in two places](#72-subtree-mounts-already-work-in-two-places)
  - [7.3 Content-addressed identity is over-pluralised](#73-content-addressed-identity-is-over-pluralised)
  - [7.4 Three laziness primitives, scoped differently](#74-three-laziness-primitives-scoped-differently)
  - [7.5 Synchronisation primitives don't form a coherent menu](#75-synchronisation-primitives-dont-form-a-coherent-menu)
  - [7.6 The wrapper-ergonomics gap](#76-the-wrapper-ergonomics-gap)
  - [7.7 Global settings vs per-input configuration](#77-global-settings-vs-per-input-configuration)
  - [7.8 "Lift this C++ data structure to a Nix attrset"](#78-lift-this-c-data-structure-to-a-nix-attrset)
  - [7.9 Composition leaks identity](#79-composition-leaks-identity)
  - [7.10 "Materialise on demand" lacks a tag](#710-materialise-on-demand-lacks-a-tag)
  - [7.11 Path-prefix lookup is implemented three times](#711-path-prefix-lookup-is-implemented-three-times)
  - [7.12 Filter ⇒ uncached: the largest single performance hole](#712-filter--uncached-the-largest-single-performance-hole)
- [8. Wishlist](#8-wishlist)
  - [8.0 The natural conclusion: finish the lazy filesystem](#80-the-natural-conclusion-finish-the-lazy-filesystem)
  - [8.1 Tier 1: address the actual pain points](#81-tier-1-address-the-actual-pain-points)
  - [8.2 Tier 2: codify and fix the algebra](#82-tier-2-codify-and-fix-the-algebra)
  - [8.3 Tier 3: generalise tecnix's primitives](#83-tier-3-generalise-tecnixs-primitives)
  - [8.4 Independent fixes upstreamable as-is](#84-independent-fixes-upstreamable-as-is)
  - [8.5 Larger-pattern abstractions](#85-larger-pattern-abstractions)
  - [8.6 What does not need to change](#86-what-does-not-need-to-change)
...
The same question generalises. Every monorepo user, every flake that calls `lib.cleanSource ./.`, every CI pipeline that evaluates a single subdirectory of a large tree, every language-server doing repeated partial evaluation pays the same costs tecnix worked around. The tecnix patches are concrete enough to read end-to-end, which makes them a tractable specimen, but the gap they reveal is general: **Nix's `SourceAccessor` layer is a half-built lazy filesystem**, and the half that's missing — subpath-aware identity, predicate-folded caching, per-byte deferral, NAR-walk avoidance — is what produces the user-visible costs in [§6](#6-empirical-evidence).

Reading order: [§2](#2-setup-the-layered-model) establishes the substrate (layered model, L2 design split, `SourceAccessor` algebra, where caches live). [§3](#3-tecnix-patch-set) inventories tecnix's patches; [§4](#4-per-patch-mapping-what-each-tecnix-patch-is-really-asking-for) maps each one to a missing upstream abstraction. [§5](#5-asymmetry-catalogue) catalogues twelve accessor-algebra asymmetries plus four symmetric operations. [§6](#6-empirical-evidence) reproduces six findings, including [§6.6](#66-filtered-sources-bypass-the-fetcher-cache--confirmed) — the filter-bypass that fires on every cold `lib.cleanSource ./.` in nixpkgs. [§7](#7-larger-patterns-nix-is-missing-or-duplicating) rolls up larger patterns. [§8.0](#80-the-natural-conclusion-finish-the-lazy-filesystem) is the unifying construction; [§§8.1](#81-tier-1-address-the-actual-pain-points)–[8.5](#85-larger-pattern-abstractions) are the concrete sub-tasks needed to assemble it. [§9](#9-limits-of-this-analysis) lists what was not verified.
...
| --- | --- | --- |
| **Monorepo evaluated against N branches/PRs that share a subtree (e.g. CI matrix or `nix flake check` over branches)** | [§5.1](#51-getfingerprint-component-composition--three-different-rules), [§6.1](#61-tree-sha-cache-miss-across-revisions--confirmed), [§7.1](#71-the-internal-tree-sha--nar-hash-projection-that-already-exists-but-isnt-surfaced) | The shared subtree is walked and re-hashed once per branch, even though the subtree SHA is identical across branches, because `getFingerprint` is keyed on the parent-rev fingerprint and not subpath-aware ([§6.1](#61-tree-sha-cache-miss-across-revisions--confirmed) reproduces with two revs) |
| **`mkDerivation { src = lib.cleanSource ./.; }`** (every nixpkgs flake) | [§6.6](#66-filtered-sources-bypass-the-fetcher-cache--confirmed), [§7.12](#712-filter--uncached-the-largest-single-performance-hole) | Fetcher cache disabled for filtered paths → re-walk + re-hash on every cold evaluation |
| **Repeated `nix develop` / `nix flake check`** on a working tree, common `src = lib.cleanSource ./.` (devshell loops, IDE LSP eval) | [§6.6](#66-filtered-sources-bypass-the-fetcher-cache--confirmed), [§5.11](#511-cachingsourceaccessor-master-only-is-partial-forwarding), [§7.5](#75-synchronisation-primitives-dont-form-a-coherent-menu) | Every reload that misses eval-cache re-walks the source: the on-disk fetcher cache is bypassed by the filter ([§6.6](#66-filtered-sources-bypass-the-fetcher-cache--confirmed)), and master's `CachingSourceAccessor` ([§5.11](#511-cachingsourceaccessor-master-only-is-partial-forwarding)) is in-process only — its lstat/readLink memo doesn't survive across `nix` invocations. Concurrent reads of one git accessor serialise on `Sync<State>` mutex ([§7.5](#75-synchronisation-primitives-dont-form-a-coherent-menu)) |
| **CI partial builds**: single subdirectory of a large monorepo | [§5.1](#51-getfingerprint-component-composition--three-different-rules), [§6.1](#61-tree-sha-cache-miss-across-revisions--confirmed), [§6.2](#62-string-interpolation-hole--confirmed), [§7.1](#71-the-internal-tree-sha--nar-hash-projection-that-already-exists-but-isnt-surfaced), [§7.2](#72-subtree-mounts-already-work-in-two-places) | If the build expression uses `src = "${input.outPath}/sub"` (string interpolation) instead of `src = builtins.path { path = input.outPath + "/sub"; }`, the [§6.2](#62-string-interpolation-hole--confirmed) hole rewrites the *whole* repo into `inputSrcs`. Even with the path-based form, [§6.1](#61-tree-sha-cache-miss-across-revisions--confirmed)'s per-rev fingerprinting forces a fresh NAR walk of the subtree on each rev's first eval |
| **Repeated `fetchTree` of same rev with same `submodules`/`exportIgnore` flags across runs** | [§3.4](#34-libfetchersgit-extensions) (gitRevUrl absent on master/DetSys) | The git ODB is cached by URL, but the assembled accessor (with submodule mounts and exportIgnore filtering) is re-built each time; tecnix's `gitRevUrl` cache memoises the assembly |
| **`nix eval --json` of a deeply-nested attrset where most leaves are paths** | [§2.4](#24-the-cli-always-devirtualizes), [§5.12](#512-the-l2-random-store-path-hides-materialisation-state) | The CLI devirtualises every path in the printed output, so the lazy-trees savings are invisible at the CLI boundary; only paths that *don't appear* in any final printed output stay virtual |
| **Cross-invocation persistence**: same source read in two `nix-instantiate` calls | [§6.6](#66-filtered-sources-bypass-the-fetcher-cache--confirmed), [§5.11](#511-cachingsourceaccessor-master-only-is-partial-forwarding) | The on-disk fetcher cache helps only when the source has a fingerprint (and isn't filtered) — see [§6.6](#66-filtered-sources-bypass-the-fetcher-cache--confirmed). Master's `CachingSourceAccessor` ([§5.11](#511-cachingsourceaccessor-master-only-is-partial-forwarding)) is in-process only and doesn't survive across `nix` invocations; DetSys and tecnix lack even that wrapper |

Each row maps to a [§5](#5-asymmetry-catalogue)/[§7](#7-larger-patterns-nix-is-missing-or-duplicating) issue and resolves under a single construction ([§8.0](#80-the-natural-conclusion-finish-the-lazy-filesystem)). The asymmetries are not isolated bugs but consequences of an accessor algebra that was never finished. Tecnix works around them with eleven user-side builtins ([§3.1](#31-eleven-new-builtins)) plus `DirtyOverlaySourceAccessor` and `gitRevUrl`; non-tecnix nixpkgs users silently pay the filter bypass ([§6.6](#66-filtered-sources-bypass-the-fetcher-cache--confirmed)) on every evaluation that misses eval-cache.
...
- **L2 — virtual store.** `MountedSourceAccessor` is a `path → SourceAccessor` table. `EvalState::storeFS` mounts at `/nix/store` and the eval-time root accessor unions it with the real POSIX filesystem. When a `fetchTree` result needs a store path before its content has been materialised, `EvalState::mountInput` mounts the input's accessor at a *virtual store path* on `storeFS`. Reads to that path during evaluation route through the mount and the underlying L1 accessor. Two competing implementations of "what is a virtual store path" exist (see [§2.3](#23-the-l2-design-split)).
...
### 3.1 Eleven new builtins

In [Shopify/tecnix:src/libexpr/primops/tectonix.cc](https://github.com/Shopify/tecnix/blob/45d9c6f4a4e7b9457bb3fe5dab18427f5671735f/src/libexpr/primops/tectonix.cc) (418 LOC),[^primops-count] parameterised by global settings, none exposed via `InputScheme`:[^no-scheme]
...
Each tecnix change maps to a missing or unsurfaced upstream abstraction. This is the document's central claim, with the supporting evidence collected in [§5](#5-asymmetry-catalogue) (asymmetries), [§6](#6-empirical-evidence) (empirical), [§7](#7-larger-patterns-nix-is-missing-or-duplicating) (larger patterns), and the recommendations enumerated in [§8](#8-wishlist).

| Tecnix patch | What's missing upstream | [§7](#7-larger-patterns-nix-is-missing-or-duplicating) pattern row | [§8](#8-wishlist) wishlist item(s) |
| --- | --- | --- | --- |
| `tectonixZoneCache_`, `__unsafeTectonixInternalTreeSha`/`Tree`/`ZoneSrc`/`ZonePath` | Tree-SHA → NAR-hash projection isn't surfaced for arbitrary git subtrees; `fetchTree` can't return a sub-tree handle | [§7.1](#71-the-internal-tree-sha--nar-hash-projection-that-already-exists-but-isnt-surfaced) internal projection unsurfaced; [§7.2](#72-subtree-mounts-already-work-in-two-places) subtree mounts already work twice; [§7.3](#73-content-addressed-identity-is-over-pluralised) content-identity over-pluralised | Tier 1 #1 + #3, Tier 3 #15 + #16 |
| `DirtyOverlaySourceAccessor`, `__unsafeTectonixInternalDirtyZones`/`ZoneIsDirty` | No generic `OverlaySourceAccessor` combinator; no dirty-introspection on workdir Inputs | [§7.6](#76-the-wrapper-ergonomics-gap) wrapper-ergonomics gap | Tier 3 #14 |
| `__unsafeTectonixInternalManifest`/`ManifestInverted` | No parse-caching for pure expressions over input contents | [§7.8](#78-lift-this-c-data-structure-to-a-nix-attrset) lift-to-Nix-attrset pattern | Tier 3 #18 |
| `lazy-trees=true` default flip | Master (post-Sergei) and DetSys disagree on what an L2 virtual store path *is*; PR #13225 proposes a fully-lazy `lazy-trees` setting upstream (random virtual store paths) but is unresolved ([§2.3](#23-the-l2-design-split)) | [§7.4](#74-three-laziness-primitives-scoped-differently) three laziness primitives, scoped differently | (design choice, not a Tier item) |
| `gitRevUrl` cache | One of [§7.3](#73-content-addressed-identity-is-over-pluralised)'s missing identity projections, surfacing as ten ad-hoc cache domains | [§7.3](#73-content-addressed-identity-is-over-pluralised) content-identity over-pluralised | [§8.4](#84-independent-fixes-upstreamable-as-is) #19 |
| `attrFingerprint` plumbing in `GitAccessorOptions` | Tree-rooted accessors can't honour `.gitattributes` without commit context | (enabling change for Tier 1 #1) | [§8.4](#84-independent-fixes-upstreamable-as-is) #21 |
| `CachingFilteringSourceAccessor::cache` → `SharedSync` | No coherent menu of synchronisation primitives | [§7.5](#75-synchronisation-primitives-dont-form-a-coherent-menu) sync primitives don't form a menu | [§8.4](#84-independent-fixes-upstreamable-as-is) #20 |
...
[^caching-source]: [NixOS/nix:caching-source-accessor.cc](https://github.com/NixOS/nix/blob/2d309b18e5a3a8734f4e64e659dac1813964c241/src/libutil/caching-source-accessor.cc). The eval-root accessor stack on master is `(AllowList?)(Caching(Union(posixFS, storeFS)))`, where AllowList wraps only in `restrictEval` or `pureEval` mode (see [eval.cc#L268-L293](https://github.com/NixOS/nix/blob/2d309b18e5a3a8734f4e64e659dac1813964c241/src/libexpr/eval.cc#L268-L293)). The wrapper also drops `getLastModified` aggregation regardless of `Provenance`, which is independently real (see [§5.6](#56-getlastmodified-aggregation--absent)) — though no caller currently relies on `getLastModified` through this stack.
...
| Method | Behaviour |
| --- | --- |
| `getFingerprint` self-check | All three return `(path, this->fingerprint)` if non-null before delegating. (Composition is asymmetric — see [§5.1](#51-getfingerprint-component-composition--three-different-rules).) |
| `getPhysicalPath` | All three: "find the first child with a physical path." |
| `union` flatness | `union(A, union(B, C))` and `union(A, B, C)` give the same observable read behaviour. (Fingerprints can differ — see [§5.1](#51-getfingerprint-component-composition--three-different-rules).) |
| `mount` ordering on overlap | For non-overlapping mountpoints, irrelevant. For overlapping, see [§5.8](#58-mount-on-duplicate-key--silent-drop). |
...
Beyond the [§5](#5-asymmetry-catalogue) catalogue, the codebase has several broader patterns each implemented multiple times in slightly different ways.
...
Eight `SourceAccessor` wrappers exist; the `ForwardingSourceAccessor` superclass is supposed to be the "delegate everything by default" base, but it forwards only 6 of ~12 virtual methods ([§5.10](#510-forwardingsourceaccessor-is-incomplete-detsys-only)). Each non-forwarded method silently drops on extension. The **missing abstraction**: a complete delegating wrapper base class that forwards every virtual method.
...
[§5.10](#510-forwardingsourceaccessor-is-incomplete-detsys-only) + [§5.11](#511-cachingsourceaccessor-master-only-is-partial-forwarding) + [§5.1](#51-getfingerprint-component-composition--three-different-rules) stack into one pattern: every wrapping/composing accessor is supposed to preserve the inner accessor's identity, but the composition rules for `getFingerprint`, `getProvenance`, `getLastModified` are inconsistently implemented. The result is silent identity-loss in some paths. **No tests** ([§2.2](#22-the-sourceaccessor-algebra)) so the asymmetries persist.

### 7.10 "Materialise on demand" lacks a tag

See [§5.12](#512-the-l2-random-store-path-hides-materialisation-state). `StorePath::random` produces paths visually indistinguishable from real CA paths. Every cross-boundary site special-cases via `storeFS->getMount`. HSM systems handle this with explicit per-file state (online/dual-state/offline) — Nix's L2 needs the equivalent.
...
[§6.6](#66-filtered-sources-bypass-the-fetcher-cache--confirmed) reproduces this and traces the call chain to nixpkgs's `lib.cleanSourceWith`. The pattern: when `fetchToStore2` is passed a `PathFilter`, it sets `fingerprint = nullopt` to avoid aliasing different filters together — logically correct given that the inner accessor's `getFingerprint` doesn't know about the filter. Two compounding effects:
...
The structural cause is the missing piece in [§5.1](#51-getfingerprint-component-composition--three-different-rules)'s `FilteringSourceAccessor` row: "predicate not folded in." Filter equivalence is undecidable in general, but **filter identity** — the closure pointer, or a hash of the lambda's lexical position plus its captured free-variable bindings — is decidable; folding that into `getFingerprint` removes the bypass without aliasing risk.

The ripple: every `mkDerivation { src = lib.cleanSource ./.; }` — the dominant nixpkgs idiom for "current directory minus the obvious junk" — pays the full NAR-walk-and-hash cost on every cold evaluation. For a multi-GB monorepo this is non-trivial wall time on every `nix-build` invocation that didn't land in the on-disk eval cache; the [§6.6](#66-filtered-sources-bypass-the-fetcher-cache--confirmed) reproduction below shows the bypass at the SQLite level.

The fix is [§8.0](#80-the-natural-conclusion-finish-the-lazy-filesystem) item **a**: give `FilteringSourceAccessor` a real subpath-aware fingerprint that includes the filter's identity, then drop the `filter ? nullopt : ...` ternary at `fetch-to-store.cc:41`.
...
The [§5](#5-asymmetry-catalogue) asymmetries, the [§6](#6-empirical-evidence) empirics, and the [§7](#7-larger-patterns-nix-is-missing-or-duplicating) patterns all point at one construction. `SourceAccessor` was designed to be a virtual filesystem where reads, hashes, and identities flow through a uniform interface. The pieces that exist (`MountedSourceAccessor`, the `fingerprint` field, the `treeHashToNarHash` projection, `mountInput`) form a coherent abstraction *if finished*. The missing pieces — subpath-aware identity, predicate-folded fingerprints, per-byte deferred reads, NAR-walk avoidance over git ODBs — are exactly what `lazyTrees` needs to be more than a single-trick optimisation.

What "finishing" means concretely. (Letters used to keep these strategic items distinct from the sub-task numbering in [§§8.1](#81-tier-1-address-the-actual-pain-points)–[8.5](#85-larger-pattern-abstractions).) Items are listed by leverage, not strict dependency: **a** is foundational and closes the most asymmetries; **b**–**f** are independent of one another and of **a**, though each composes well with **a**.
...
**a. Subpath-aware fingerprint everywhere.** Today every accessor has one constant `fingerprint` field; `getFingerprint(path)` returns it regardless of `path`. Replace with `getFingerprint(path) = (canonical-prefix, fingerprint-of-prefix)`:
  ...
  - `MountedSourceAccessor::getFingerprint(path)` resolves to the appropriate child mount's subpath fingerprint (already half-implemented in [§5.1](#51-getfingerprint-component-composition--three-different-rules)'s `MountedSourceAccessor` row).
  ...
  This unifies [§8.1](#81-tier-1-address-the-actual-pain-points) #1 (tree-SHA on git), [§8.2](#82-tier-2-codify-and-fix-the-algebra) #4 (predicate folding on Filter), and [§8.2](#82-tier-2-codify-and-fix-the-algebra) #5 (component composition on Union) into one coherent change. It's the highest-leverage item: **[§5.1](#51-getfingerprint-component-composition--three-different-rules), [§6.1](#61-tree-sha-cache-miss-across-revisions--confirmed), [§6.6](#66-filtered-sources-bypass-the-fetcher-cache--confirmed), [§7.12](#712-filter--uncached-the-largest-single-performance-hole) all close simultaneously.** ([§5.10](#510-forwardingsourceaccessor-is-incomplete-detsys-only)'s `ForwardingSourceAccessor` gap is a separate fix: [§8.2](#82-tier-2-codify-and-fix-the-algebra) #10.)

**b. Tree-SHA → NAR-hash projection generalised across all opened git ODBs.** `treeHashToNarHash` already exists in the fetcher cache for the tarball ODB. Make it also consult the per-URL `gitv3` ODBs and any other git ODB the process knows about. Tecnix's `tectonixZoneCache_` becomes redundant. Closes [§7.1](#71-the-internal-tree-sha--nar-hash-projection-that-already-exists-but-isnt-surfaced).

**c. NAR-hash-free CA path for git-rooted content.** Master's [`Store::addToStore`](https://github.com/NixOS/nix/blob/2d309b18e5a3a8734f4e64e659dac1813964c241/src/libstore/store-api.cc#L161-L193) at [#L178-L181](https://github.com/NixOS/nix/blob/2d309b18e5a3a8734f4e64e659dac1813964c241/src/libstore/store-api.cc#L178-L181) reads `case FileIngestionMethod::Git: fsm = FileSerialisationMethod::NixArchive;` with the comment "Use NAR; Git is not a serialization method" — so even when the user requests `Git` ingestion, the serialisation falls back to NAR via `dumpPath` at [#L190](https://github.com/NixOS/nix/blob/2d309b18e5a3a8734f4e64e659dac1813964c241/src/libstore/store-api.cc#L190). The store-path computation [already accepts](https://github.com/NixOS/nix/blob/2d309b18e5a3a8734f4e64e659dac1813964c241/src/libstore/store-dir-config.cc#L106-L113) a Git tree SHA-1 or SHA-256 in `makeFixedOutputPath`, but `addToStore` never feeds it a tree-SHA. Add a `FileSerialisationMethod::Git` path that mints the store path directly from the tree SHA. With **a** alone, [§6.1](#61-tree-sha-cache-miss-across-revisions--confirmed)'s "two NAR walks for one shared subtree" becomes one walk total: both revs key on the same subtree-SHA fingerprint, so whichever rev is evaluated first writes the row, and any rev sharing the subtree thereafter is a cache hit. With **a** + **c** + `FileIngestionMethod::Git`, even that first walk drops to a tree-SHA → store-path mint.

**d. Per-attribute and per-byte laziness, unified.** [§7.4](#74-three-laziness-primitives-scoped-differently) catalogues three already-existing laziness primitives (`mountInput` for whole trees, `LazyAttr` master-only for per-attribute already memoising `lazyRevCount`, and `makeLazyNarAccessor` for per-byte). Unify them under one `Lazy<T>` abstraction tied to `EvalState`: `getLastModified()` becomes a lazy attribute (today combinators return `nullopt` immediately, [§5.6](#56-getlastmodified-aggregation--absent)); `readFile` of a virtual path streams from the underlying ODB without a NAR-sink round-trip unless the consumer demands one; `LazyAttr` extends to user-controlled `fetchTree`-result attributes (e.g. per-subpath tree SHA — currently only available via tecnix's `__unsafeTectonixInternalTreeSha`).

**e. Lockfree concurrent reads on the source side.** [§7.5](#75-synchronisation-primitives-dont-form-a-coherent-menu) / [§1.1](#11-why-this-matters-beyond-tecnix)'s third workload row: `GitSourceAccessor` holds a `Sync<State>` mutex (at [DetSys:git-utils.cc#L815](https://github.com/DeterminateSystems/nix-src/blob/11f3aff904f84ae612e36e8bc578ac421fca74fa/src/libfetchers/git-utils.cc#L815), exclusive in both `lock()` and `readLock()` because `Sync<>` aliases both to `unique_lock<mutex>`) around every read. Replace with `SharedSync` for read-mostly state, or split `state_` into a libgit2-handle (mutex-protected for libgit2 thread-safety) and a derived read cache (lockfree). Sub-tasks: [§8.2](#82-tier-2-codify-and-fix-the-algebra) #13 (fix the `Sync<T>::readLock()` misnomer) and [§8.4](#84-independent-fixes-upstreamable-as-is) #20 (tecnix's `SharedSync` wrap of `CachingFilteringSourceAccessor::cache`).

**f. Eliminate the CLI devirtualise-everything default ([§2.4](#24-the-cli-always-devirtualizes)).** `nix eval --json` of a deeply-nested attrset materialises every path that appears in output. Make CLI devirtualisation opt-in or scoped to the requested attribute path. Without this, the laziness work above is invisible at the CLI boundary.

The [§5](#5-asymmetry-catalogue)/[§7](#7-larger-patterns-nix-is-missing-or-duplicating) issues outside this construction — `mount`-on-duplicate ([§5.8](#58-mount-on-duplicate-key--silent-drop)), `OverlaySourceAccessor` combinator ([§7.6](#76-the-wrapper-ergonomics-gap)), `Provenance` promotion ([§7.9](#79-composition-leaks-identity)) — are independently useful but not load-bearing for the lazy-filesystem core. Keep as [§8.2](#82-tier-2-codify-and-fix-the-algebra)/[§8.3](#83-tier-3-generalise-tecnixs-primitives) work.
...
- `${(fetchTree input).outPath}/sub` in `derivation` no longer rewrites the whole repo into `inputSrcs` — but this is closed by [§8.1](#81-tier-1-address-the-actual-pain-points) #2 (subpath-aware `ensureLazyPathCopied` / `devirtualize`), which is orthogonal to [§8.0](#80-the-natural-conclusion-finish-the-lazy-filesystem). The two compose: #2 makes the materialisation subpath-scoped, **a** makes the resulting cache key share across revs whose subtree SHA matches.
...
[§§8.1](#81-tier-1-address-the-actual-pain-points)–[8.5](#85-larger-pattern-abstractions) below are the concrete sub-tasks needed to assemble this construction, plus independent fixes that don't depend on it.

### 8.1 Tier 1: address the actual pain points

1. **Tree-SHA-aware fingerprint on git accessors.** `getFingerprint("/sub")` should return the *subtree* SHA, not `git:C;e?;l?`. Collapses [§6.1](#61-tree-sha-cache-miss-across-revisions--confirmed)'s duplicate cache rows. Tecnix's `getSubtreeSha` is the primitive; `attrCommitRev`/`attrPathPrefix`/`attrFingerprint` plumbing keeps `.gitattributes` correct.
...
4. **Fold predicates into `FilteringSourceAccessor::getFingerprint`** ([§5.1](#51-getfingerprint-component-composition--three-different-rules)).
5. **Compose component fingerprints in `UnionSourceAccessor::getFingerprint`** ([§5.1](#51-getfingerprint-component-composition--three-different-rules)).
6. **`MountedSourceAccessor` and `UnionSourceAccessor` consult `this->provenance` first** ([§5.2](#52-getprovenance-self-check--asymmetric)).
7. **Override `pathExists` on `MountedSourceAccessor` and `UnionSourceAccessor`** to forward to inner overrides ([§5.3](#53-pathexists-override--asymmetric-l)) so source-level optimisations propagate through composites.
8. **Define semantics for duplicate `mount`** ([§5.8](#58-mount-on-duplicate-key--silent-drop)).
9. **Implement `getLastModified` aggregation** ([§5.6](#56-getlastmodified-aggregation--absent)).
10. **Forward `getFingerprint`, `getProvenance`, `getLastModified`, `invalidateCache` through `ForwardingSourceAccessor`** ([§5.10](#510-forwardingsourceaccessor-is-incomplete-detsys-only), [§5.11](#511-cachingsourceaccessor-master-only-is-partial-forwarding)).
11. **Differentiate virtual from real CA paths** ([§5.12](#512-the-l2-random-store-path-hides-materialisation-state), [§7.10](#710-materialise-on-demand-lacks-a-tag)).
12. **Add contract tests for the combinator algebra** ([§2.2](#22-the-sourceaccessor-algebra)). Zero tests today.
13. **Fix `Sync<T>::readLock()` naming** ([§7.5](#75-synchronisation-primitives-dont-form-a-coherent-menu)) — it's not a shared lock under `Sync<T>`.

### 8.3 Tier 3: generalise tecnix's primitives

14. **Generic `OverlaySourceAccessor` combinator** with whiteouts ([§7.6](#76-the-wrapper-ergonomics-gap) / [§6.3](#63-whiteout-correctness-in-dirtyoverlaysourceaccessor--verified)).
15. **Subtree-as-Input syntax** ([§7.2](#72-subtree-mounts-already-work-in-two-places) / [§7.7](#77-global-settings-vs-per-input-configuration)). `fetchTree { ... subdir = "/areas/tools/tec"; }` returning a content-addressed sub-result, mounted via the existing submodule machinery.
16. **`fetchTree` returns `nPath` value** ([§7.2](#72-subtree-mounts-already-work-in-two-places)), so `(fetchTree input) / "sub"` works via existing `SourcePath::operator/`.
17. **Promote `Provenance` from `Xp::Provenance`** ([§7.9](#79-composition-leaks-identity)). Surface `(fetchTree input).provenance`.
18. **Parse-caching helpers on `fetchTree` results** ([§7.8](#78-lift-this-c-data-structure-to-a-nix-attrset)).
...
23. **Unified content-identity primitive** ([§7.3](#73-content-addressed-identity-is-over-pluralised)).
24. **Unified laziness primitive** spanning whole-tree, per-attribute, and per-byte ([§7.4](#74-three-laziness-primitives-scoped-differently)).
25. **Sync-primitive convention** — recipe for "what kind of lock for this kind of state" ([§7.5](#75-synchronisation-primitives-dont-form-a-coherent-menu)).
26. **Path-prefix lookup combinator** generalising Mounted/Filtering's prefix and `RemoteFSAccessor::narHashes` ([§7.11](#711-path-prefix-lookup-is-implemented-three-times)).

### 8.6 What does not need to change

The mount/devirtualize *architecture* — [§8.0](#80-the-natural-conclusion-finish-the-lazy-filesystem) is internal to the existing layers. The `SourceAccessor` *interface signature* — getFingerprint remains `(CanonPath) → (CanonPath, optional<string>)`, just with subpath-aware semantics. The flake input model — additive subtree-handle is sufficient. The Nix language — no new syntax. The daemon protocol. The on-disk sqlite schema — existing tables accommodate new fingerprint shapes; only cache *contents* shift.

What *does* change: [§8.0](#80-the-natural-conclusion-finish-the-lazy-filesystem) item **c** adds a `FileSerialisationMethod::Git` branch to `Store::addToStore` (one new case in the switch, plus a new code path that calls `makeFixedOutputPath` with a tree-SHA instead of running `dumpPath`). And cache keys for filtered sources gain a filter-identity component, so old `sourcePathToHash` rows that were never written (because of [§6.6](#66-filtered-sources-bypass-the-fetcher-cache--confirmed)) start accumulating. No rewrite required: old rows remain valid, new rows fill in over time.
...
- **No benchmark of master's eager-NAR-hash cost vs DetSys's `StorePath::random`** at tecnix scale. [§2.3](#23-the-l2-design-split)'s cost claim ("every zone mount pays one full NAR-hash walk of the subtree, regardless of materialisation") is reasoned from code paths, not measured. Building Nix from source would dominate the session.
- **No reproducers for the dormant asymmetries in [§5](#5-asymmetry-catalogue).** Each requires synthetic C++ accessors with custom fingerprints/provenance, not user-level Nix.
