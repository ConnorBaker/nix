...
The cache decorator law inventory is implemented for `CachingSourceAccessor` in `src/libutil-tests/caching-source-accessor.cc` (`C1`/`C2`/`C3`/`C5`/`C6`; `C4` belongs to the separate predicate-cache family). The `AccessControl` `A1–A6` inventory remains design-level because the eval-root allow list is a stateful effect rather than a pure operator ([§5](#5-design-rationale)).
...
- *CoW store (btrfs/xfs, the best case):* `tryCloneFile`'s `FICLONE` shares the unchanged extents outright, so assembly is O(changed bytes) regardless of tree size. The `dirty-tree-assemble-cow` NixOS VM test (store on a real btrfs partition, 200 files × 64 KiB, one edit) measures **`btrfs filesystem du` exclusive ≪ total** (the test asserts `exclusive < total/4`): only the one edited file's extents are exclusive, the unchanged majority sharing the base's extents, so editing one file allocates new data blocks for that one file rather than re-copying the tree.
- *non-CoW store (extN/tmpfs):* the unchanged files are hardlinked (metadata only, no byte copy). The `nix-store-benchmarks` `BM_FullCopyTree` vs `BM_AssembleFromBase` contrast (256 files × 256 KiB; `createTempDir`, typically tmpfs in CI) shows assembly the faster of the two — order ~1.7× in one local run — with the gap growing with tree size; run the benchmark for current numbers on your filesystem. (With *tiny* files on tmpfs the two are at parity — a memcpy is already free — which is why the bench uses larger files and the CoW proof lives in the VM, where a real filesystem participates.)
...
**The re-hash guard (the load-bearing soundness obligation).** Unlike `registerLinkedCAPath`'s copy-once-link-N — which links *byte-identical* siblings, where equality holds by construction — the assembler builds a **novel** tree, so a wrong base, a stale unchanged file, or a mis-classified path would produce content that does not match the narHash naming it. `ValidPathInfo::makeFromCA` stores `narHash` *verbatim* (it never re-hashes the bytes), so the ordinary path-derivation check would not catch this. The assembler therefore re-hashes the assembled tree (NAR, SHA-256) and **names the path from that measured hash** — so the registered path always matches its own content. The seam (`ensureLazyPathCopied`) then compares that content-derived name against the one the evaluator minted (`realPath`) and falls back to a full copy on any disagreement, so a mis-assembled tree is never registered *as* `realPath`. (The unit test exercises the stricter `expectedPath`-throws variant, where the assembler rejects the mismatch internally before `registerValidPath`; production passes no `expectedPath` and relies on the seam's equal-or-fall-back check.) This re-hash is the same single read the hashing already requires (there is no separate dryRun), so it is not a second walk; it caps the win at "the byte-copy of the unchanged majority is saved", not "the read is avoided". The saving is therefore disk footprint + I/O (and, on a CoW store, near-total): illustratively, on a 300-file tree with one edit, 299 files would be shared with the base and one rewritten (the tested trees are smaller — n=40 functional, 200 in the CoW VM, ≤256 in the bench). The guard is verified by a negative unit test (`AssembleFromBaseExpectedPathMismatchThrows` in `register-assembled-ca-path.cc`): an assembly whose content does not hash to the expected name must throw and register nothing.
...
- **The one place that discards the context.** `builtins.unsafeDiscardStringContext` (`primops/context.cc`) is the inverse hazard: it *removes* the very string context the universal devirtualise sweep keys off, so a deferred placeholder could not be rewritten after the fact. It therefore routes the body through `resolveAndRewrite` to materialise and substitute the placeholder **before** dropping the context (a no-op for a plain string) — otherwise the fake `/<base32>` render would survive as a dangling literal baked into a `.drv` builder arg under impure eval / `nix repl`. Tested through the real primop (`boundary-audit-discard-context.cc`).
...
- **The regression gate.** `src/libexpr-tests/boundary-audit-*.cc` — 24 files, 37 cases across the audited sites; the search surface for a new bypass is `git grep "string_view()" / ".getString()" / 3-arg forceString` in CLI/primops code.
...
The strict wins are cargo coalescing + copy-once-link-N, interpolation-without-copy, the dirty-workdir base-plus-overlay assembly, and the rev-pinned defer (neither DetSys nor tecnix does either of the last two — both re-copy a dirty tree whole and re-walk a locked input on a cold store). These last two are *absence*-of-feature observations, not permalinkable lines: they follow from the cited eager-random-mount `mountInput`[^detsys-mountinput] (no deferred-copy path) and the git-input path never consulting `treeHashToNarHash`[^detsys-treehash] (so a locked input re-walks) — neither tree has an assembler or a rev-pinned-defer code path to cite. The pure-eval flake-input row is parity. The one axis where DetSys is ahead is the orthogonal async `.drv` writer ([§6.5](#65-design-boundaries-deliberately-not-crossed)), which is unrelated to source materialisation.
