...
- DetSys's **`AsyncPathWriter`** (`nix-src/src/libstore/async-path-writer.{hh,cc}`) — the existing proof that the `.drv` *write* can be moved off the eval thread, **and** (in its disabled `#if 0` block) the first attempt at the batched store transaction this design completes.
...
- [4. The design](#4-the-design)
  ...
  - [4.3. The batched store transaction + the narHash friction fix](#43-the-batched-store-transaction)
...
1. **Build.** `buildPaths`/`ensurePath`/`buildDerivation` in libstore require the `.drv` valid (the daemon reads it). The CLI build entry points are where DetSys already places `waitForPath`/`waitForAllPaths` ([§3](#3-detsys-asyncpathwriter)); this design reuses the identical barrier set.
2. **`computeFSClosure`** — the `DrvDeep` arm of step 3 (`primops.cc:1855`). A whole-closure dependency forces the dependency `.drv` (and its closure) valid *during eval*. This is the one in-eval boundary; it must flush the named `drvPath` (and transitively its pending inputs) before the closure walk.
3. **`import (drv.drvPath)` / `readDerivation` / `readInvalidDerivation`** (`store-api.cc:1168` `readDerivationCommon`). Reading a `.drv` back as a Nix value requires its bytes.
4. **CLI/C-API observation** — `nix show-derivation`, `nix derivation show`, copying a `.drv` to a binary cache, `--add-root` on a `.drv`. These are the `waitForAllPaths` sites.
...
- a **bulk flush at the artificial boundary** (end-of-eval / `waitForAllPaths`, and each build submission) — the batched fast path ([§4.3](#43-the-batched-store-transaction));
- a **targeted flush-on-read** (`waitForPath(drvPath)` / a guard inside `readInvalidDerivation` and the `DrvDeep` arm) for the rarer in-eval read-back, transitively flushing that `.drv`'s pending input `.drv`s first.

The flush-on-read guard is the same *shape* as the source-side boundary audit ([PROPOSAL.md §6.2](./PROPOSAL.md#62-the-addpath-source-virtualisation-boundary)): a finite, enumerable set of sites that consume the artifact, each of which must drain before consuming. The audit surface is `git grep` for `readInvalidDerivation` / `readDerivation` / `computeFSClosure` callers reachable from eval.

### 4.3. The batched store transaction

At the bulk boundary, instead of N independent `writeDerivation` calls (N `addToStoreFromDump` round-trips, N transactions), flush all pending `.drv`s — and all deferred source copies — through **one** registration:

- `LocalStore::registerValidPaths(ValidPathInfos)` (`local-store.cc:938`) is a **single `SQLiteTxn`, two-phase**: loop 1 `addValidPath` every path, loop 2 wire `AddReference`, then `topoSort` for cycles (`:952-991`). The two-phase structure means **intra-batch references are legal** — a `.drv` in the batch may reference another `.drv` or a source in the *same* batch. A whole dependency subtree registers atomically.
- The daemon equivalent is `addMultipleToStore` (`WorkerProto::Op::AddMultipleToStore = 44`, `store-api.cc:195`), which drives `processGraph` — topologically ordered, reference-aware, skipping already-valid paths.

**The narHash friction, and its fix.** This is DetSys's `#if 0` FIXME. A `.drv` is `text:sha256` over *flat* bytes (`Raw::Text`, `FileSerialisationMethod::Flat`, `derivations.cc:150-151`). But `addMultipleToStore` takes `ValidPathInfo`, and `LocalStore::addToStore(info, source)` re-hashes the incoming NAR and throws on `hashResult.hash != info.narHash` (`local-store.cc:1062`). There is no `nar:sha256` in hand for a flat text object. Two resolutions, in order of preference:

1. **Compute the per-item `narHash` when building the batch.** For a `.drv` ATerm (kilobytes) a NAR dump + SHA-256 is negligible, and it is the same single pass `addToStoreFromDump` would do internally. The batch then satisfies the existing `addMultipleToStore` contract unchanged. *This is the v1: it resolves the FIXME with no store-API change.*
2. (Future) a `addMultipleToStore` variant whose `ValidPathInfo` may carry a CA/text hash and skip the NAR verify when the CA method is `Raw::Text` — avoids even the per-item NAR dump. Deferred because (1) is already cheap and (2) widens a security-relevant verify path (R4: the verify is what makes `addToStore(info, source)` safe against a lying source).

Deferred **source** copies flush in the *same* transaction: a source's `inputSrcs` entry is named by its narHash (intrinsic, already computed), so its `ValidPathInfo` is fully determined; the copy supplies the bytes. This is the cross-derivation batching the per-`derivationStrict` `outPathsOf` cannot do — N derivations sharing sources, or a deep dependency chain, copy and register in one pass.
...
- **Intra-batch references resolve.** *Obligation:* a batched `.drv` referencing a same-batch source/`.drv` must register without a "missing reference" error. *Discharge:* the two-phase `registerValidPaths` ([§4.3](#43-the-batched-store-transaction)) inserts all paths before wiring any reference; `topoSort` orders them and detects cycles. Verified by reading `local-store.cc:952-991`.
- **Flush-on-read covers every in-eval read-back.** *Obligation:* no eval-time consumer reads a pending `.drv`'s bytes. *Discharge:* the enumerated boundary set ([§4.2](#42-resolution-boundaries)) with `git grep`-able audit surface; the `DrvDeep` `computeFSClosure` arm and `readInvalidDerivation` are the only in-eval byte consumers, both gated by a `waitForPath` that transitively flushes inputs.
...
- The unified flush is therefore: at a build/observation boundary, **batch-copy all deferred sources and batch-register all pending `.drv`s in one transaction**, with intra-batch references ([§4.3](#43-the-batched-store-transaction)).
...
|---|---|---|
| `nix build --dry-run` / `nix eval .#x.drvPath`, source-heavy flake | N source copies + (read-only: 0 `.drv` writes; non-read-only build path: N `.drv` writes, N txns) | 0 source copies, 0 `.drv` writes — nothing flushed |
| `nix build` of 1 target, M-derivation closure, all built from source | M `.drv` writes (M txns) + per-drv source copies | M `.drv` writes + source copies in **1 batched txn**; writes overlap eval if async |
| `nix build` where the target is substitutable from a cache | full instantiation: M `.drv` writes + source copies, then substitution makes most unnecessary | only the `.drv`s/sources on the *local-build* path flush; substituted-away closure never materialises |
| Deep dependency chain (A→B→…→Z) | each `.drv` write a separate txn | one batched txn at the boundary, intra-batch refs |

**The workload-shape risk (R5/R6, stated as a blocker not an afterthought).** Deferral can *un-fuse* deliberately fused work and lose. Two shapes to measure before defaulting on:

- **Build-everything-from-source:** every `.drv` is written anyway, so the only delta is write *coalescing* (N txns → 1) and async overlap. This should be neutral-to-positive — batching reduces fsync/round-trip count — but the async thread adds a lock-handoff per `.drv`; on a tiny closure the handoff could dominate. Measure on a small closure.
- **Instantiate-many / build-few / substitute:** the biggest win (never materialise the substituted-away closure), but only if the substitutability query happens *before* the flush. If the build scheduler forces every `.drv` valid before querying substituters, the deferral buys nothing. **This is the prerequisite to verify (R1): does the build path query substituters before or after it needs the `.drv` on disk?** If after, a restructure is needed and the win is future-work, not v1.

The source-materialisation work already learned this lesson the expensive way (an eval-trace cache that was hot-7×-faster on one workload and cold-8.6×-slower on another); this design inherits the discipline of gating behind a setting and measuring both shapes, not defaulting blind.
...
| `.drv` write timing | bg thread, per-item loop | bg thread, **1 txn per flush** | skipped until a boundary reads it |
| Source copy | eager (unchanged) | **deferred, same batch** | deferred |
| Win | hide write *latency* | + reduce *txn/round-trip count*; cross-drv source batching | + never materialise substituted-away closures |
| Resolution boundary | `waitForPath` per CLI site | same + end-of-eval bulk flush | same + flush-on-read guard |
| Blocker | none (shipped) | narHash friction ([§4.3](#43-the-batched-store-transaction)) — solved by per-item narHash | substitutability-before-flush ([§7](#7-performance-characteristics)) — needs verification |

They are **complementary, not alternatives.** The ideal end state is async + batched: DetSys's `#if 0` block *enabled* and running on the worker thread, with the narHash friction resolved and source copies folded into the same transaction. This design is "port DetSys's async harness, then complete the batch it disabled, then extend the batch to sources."

Relative to DetSys specifically: DetSys defers the `.drv` write but eagerly copies sources and writes per-item; this design defers both and batches both into one transaction at one boundary. Relative to upstream/our-tree: neither has any of it (`writeDerivation` is synchronous on our branch — `derivations.hh:602`, single overload, no `AsyncPathWriter`).
...
- **Not a daemon-protocol change (v1).** The batched flush uses the existing `addMultipleToStore` (op 44) and `registerValidPaths`; the per-item-narHash fix is client-side. The optional text-CA `addMultiple` variant ([§4.3](#43-the-batched-store-transaction) item 2) *would* be a protocol addition and is explicitly deferred.
...
Per [REVIEW-DISCIPLINE.md](./REVIEW-DISCIPLINE.md), the load-bearing claims that a review must *simulate* (R1), not cite:

1. **(R1, prerequisite blocker) Substitutability ordering.** Does the build scheduler (`buildPaths` → `DerivationGoal`) query substituters for a derivation's outputs *before* it requires the `.drv` valid on disk? If after, the "never materialise substituted-away closures" win ([§7](#7-performance-characteristics)) is future-work and v1 reduces to async+batched-write only. *This is the single most important thing to verify before sizing the win.* Walk: `nix build` entry → `Store::buildPaths` → goal creation → where `readDerivation`/`isValidPath(drvPath)` is first called relative to `querySubstitutablePaths`.
2. **(R1) The `DrvDeep` in-eval flush frequency.** How common is the whole-closure `computeFSClosure` edge (`primops.cc:1855`) in real nixpkgs eval? If frequent, the flush-on-read fires often and erodes the batching win. `Built`/`ensureSlot` (the common edge) does *not* flush (simulated, [§1](#1-what-derivationstrict-does-today)); confirm `DrvDeep` arises only from `unsafeDiscardOutputDependency` and similar, not from ordinary `${pkg}` interpolation.
3. **(R5) Latency floor of the per-item narHash.** Confirm a NAR dump + SHA-256 of a typical `.drv` ATerm is negligible against the `addToStoreFromDump` it replaces — i.e. the friction fix ([§4.3](#43-the-batched-store-transaction) item 1) is genuinely free, not a hidden second pass.
4. **(R4) Blast radius.** This design changes *when* `.drv`s and sources are written, not *what* — `drvPath`, `outPath`, and the `.drv` contents are byte-identical to today. So there is no rebuild scope (no content-hash change). Confirm by R2: the `drvPath` is `hash(unparse(drv))` in both the eager and deferred paths, over the same `drv` struct.
5. **(R6) Alternatives.** (a) DetSys's async-only writer — rejected as *insufficient*, not wrong: it leaves source copies eager and writes per-item, so it captures latency but not throughput. We adopt its harness. (b) A daemon-side deferred-write registry — rejected: the pending state needs `EvalState` context the daemon lacks (the same eval/daemon split that bounds the source design); the flush must be client-side before build submission. (c) Doing nothing and relying on OS page-cache to make per-item writes cheap — rejected on R5: the cost is fsync + SQLite txn commit per `.drv` (`registerValidPaths` syncs, `local-store.cc:945`), which the page cache does not amortise.

A review of this document that does not walk items 1 and 2 against the source is a citation audit, not an adversarial review (R8).
